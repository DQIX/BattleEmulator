...
FLEEは現 `Main` の `skipTurn` の順序を正本とし、麻痺・inactiveによる追加の入口制約を置かない。
味方枠ではFLEE判定をstatus処理より先に行う。詳細は第1節に定める。
...
**FLEEの合法性と意味は、現 `Main` の `skipTurn` を正本とする。**

- PoCの `Selectable(FLEE)` には、麻痺・inactiveによる追加の入口制約を置かない。
- 味方行動枠に到達して準備済み行動が `FLEE_ALLY` なら、麻痺・inactiveの味方status処理より先に `skipTurn=true` となる。
  その味方枠では麻痺残りの減算・解除判定、inactive解除、味方action case、味方action配列への追加、味方行動後のアクロ減算を実行しない。
- 敵が先攻してそのターン中に麻痺・inactiveを付与していても、FLEEの味方枠では上記status処理へ戻らない。
  敵側で既に起きた状態変更は保持したまま次へ進む。
- FLEEは敵へ直接ダメージを与えなくても、敵行動とターン末尾・cameraを通した後の乱数位置を変える有効な選択肢なので削除しない。
- PoCは眠りなしの入力に限定するため、現 `Main` に存在しないFLEE専用の眠り合法性を追加しない。
...
このFLEE規則について、proof用の再実行入口が `Main` に別のstatus再判定を足してはならない。
RuleProgramも `skipTurn` の判定順序をそのまま表し、現 `Main` と同じ生状態遷移を検査する。
...
PoCで必要な実状態や再実行用の状態は、可逆なビットパックで保持する。`uint64_t[3]`（24byte）はその設計案である。
ハッシュ値を状態として扱わない。
証明用のmode化で無効タイマー等を省く写像は、この可逆保存とは別である。
横展開で資源や状態を追加する場合、実状態の保存容量も再検証する。
現行C++でビットパックされているのはターン番号・カメラ等を持つ `NowState` であり、HP等は `Player` の各フィールドにある。
本書の全体の格納形式を実装済みとは扱わない。資源24bit＋六つのmodeの符号16bitという容量計算も、生の全フィールドを復元する形式の実装確認ではない。
...
| 行動の合法性 | MPが2以上・3以上、薬草が1以上、ACROの必殺・アクロ状態条件など。FLEEにはPoC上の追加入口条件を置かない |
...
FLEEも同じ根から始め、味方枠では `skipTurn` をstatus処理より先に適用する。
...
FLEEについては、味方枠の `skipTurn` が麻痺・inactive処理より先であること、
敵先攻で状態異常が付いた場合もその味方枠で解除・減算へ入らないこと、
その後も敵行動・ターン末尾・cameraの規定処理を維持することを別々に確認する。
未対応の眠り処理をFLEE専用規則として推測で追加しない。
