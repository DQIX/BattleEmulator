...
namespace MemConst {
    ...
    constexpr int32_t NODE_COUNT =
        ActionBruteForcerConst::MAX_NODES;

    constexpr int32_t NODE_BUF_COUNT = 2;

    constexpr int32_t NODE_BUF_BYTES =
        sizeof(Node) * NODE_COUNT * NODE_BUF_COUNT;

    // SearchOutput
    constexpr int32_t SEARCH_OUTPUT_BYTES =
        sizeof(const Node*) * NODE_COUNT
      + sizeof(int);

    // Pool 系
    constexpr int32_t NODE_POOL_SIZE =
            ActionSearcher::MAX_LAYER * ActionSearcher::MAX_LAYER;

    constexpr int32_t ACTION_POOL_SIZE =
        NODE_POOL_SIZE * ActionBruteForcerConst::CONST_MAX_DEPTH;

    constexpr int32_t POOL_BYTES =
    ...
    // 合計
    constexpr int32_t TOTAL_BYTES =
          NODE_BUF_BYTES
        + SEARCH_OUTPUT_BYTES
        + POOL_BYTES;

    constexpr int32_t GiB = 1024ull * 1024 * 1024;
