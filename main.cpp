...
const Player copiedPlayers[2] = {
	// プレイヤー1
	{
		309, 309, BattleEmulator::BARUBOROSU_EQUIPPED_ATK, BattleEmulator::BARUBOROSU_EQUIPPED_ATK, 299, 299, 193, 193, 234, 168, // 最初のメンバー
	...
	for(int i = 0; i < result.position; ++i){
		...
		if(isEnemy){
			...
			if((action & BattleEmulator::ACTION_EQUIPMENT_CHANGED) != 0){
				if((action & BattleEmulator::ACTION_GANNAN) != 0){
					equipmentChange = "gannan";
				}else if((action & BattleEmulator::ACTION_BARE_HANDS) != 0){
					equipmentChange = "off";
				}else{
					equipmentChange = "on";
				}
