...
#define YO2_SEARCH_BUDGET_MS 2000
        ...
        // Do not trust a search-internal terminal state as output. Rebuild the
        // whole action sequence and replay it from the original battle start.
        ...
        // Primary output: the authoritative replay table comes before summaries.
        std::cout << dumpTable(replayResult, replayGene, turns) << std::endl;

        SearchAlgorithm::State replayFinalState{};
        replayFinalState.players[0] = replayPlayers[0];
        replayFinalState.players[1] = replayPlayers[1];
        replayFinalState.position = replayPosition;
        replayFinalState.nowState = replayNowState;
        if (!SearchAlgorithm::SameState(replayFinalState, searchResult.finalState)) {
            std::cerr << "Search replay verification failed: search and replay final states disagree." << std::endl;
        }
