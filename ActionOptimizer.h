...
public:
	// The final search uses a wall-clock millisecond budget, including replay.
	static Genome RunAlgorithm(const Player players[2], uint64_t seed, int turns, int budgetMs,
	...
	static std::pair<int, Genome> RunAlgorithmAsync(const Player players[2], uint64_t seed, int turns,
													int budgetMs, int actions[350], int numThreads,
													bool dropbug);
