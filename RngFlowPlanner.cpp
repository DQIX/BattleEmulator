...
namespace rngflow {
    ...
    namespace {
        struct StaticDamageEnvelope {
            int enemyHp = 0;
            int nonCriticalHitUpper = 0;
            int criticalBonusUpper = 0;
            int position = 0;
            bool acrobaticStar = false;
            int acrobaticStarTurn = 0;

            [[nodiscard]] int DamageUpperAt(const int turns) const {
                if (turns <= 0) return 0;

                // One hero command can deal at most one hit. A second player-owned hit
                // can only be an Acrobatic Star counter. Treat every active Star turn
                // as a successful counter and pretend a new Star can always be
                // activated as soon as useful. This intentionally overestimates real
                // damage and is therefore safe for one-way UNSAT pruning.
                const int initialCounterCapacity = acrobaticStar
                                                       ? std::min(turns, std::max(0, acrobaticStarTurn))
                                                       : 0;
                int hitUpper = turns + initialCounterCapacity;
                const int maxActivations = (std::max(0, turns - initialCounterCapacity) + 5) / 6;
                for (int activations = 1; activations <= maxActivations; ++activations) {
                    const int heroHits = std::max(0, turns - activations);
                    const int counterHits = std::min(
                        turns, initialCounterCapacity + 6 * activations);
                    hitUpper = std::max(hitUpper, heroHits + counterHits);
                }

                const int criticals = StaticCriticalSlotUpperBound(position, turns);
                return nonCriticalHitUpper * hitUpper
                       + criticalBonusUpper * std::min(criticals, hitUpper);
            }
        };

        template<typename PlayerLike>
        [[nodiscard]] StaticDamageEnvelope MakeStaticDamageEnvelope(
            const PlayerLike &hero, const PlayerLike &enemy, const int position) {
            // Safe damage envelope for the supported yo2 player actions.
            // PhysicalDamage2Rng is strictly below atk1 + spread + 1. Round upward on
            // purpose: this is an upper bound, not a damage prediction.
            const double atk1 = hero.atk * 0.5 - enemy.def * 0.25;
            const double spread = std::max(0.0, atk1) * 0.0625;
            const int physicalNonCriticalUpper = std::max(
                0, static_cast<int>(std::ceil(std::max(0.0, atk1) + spread + 1.0)));
            ...
            const int criticalHitUpper = std::max(
                nonCriticalHitUpper,
                static_cast<int>(std::ceil(std::max(0, hero.atk) * 1.05)));
            return {
                std::max(0, enemy.hp),
                nonCriticalHitUpper,
                criticalHitUpper - nonCriticalHitUpper,
                position,
                hero.acrobaticStar,
                hero.acrobaticStarTurn,
            };
        ...
        [[nodiscard]] int StaticOptimisticKillTurnsBattle(
            const BattleEmulator::SearchState &root, const int maxTurns) {
            const int turns = std::clamp(maxTurns, 0, kMaxPlanTurns);
            const StaticDamageEnvelope envelope = MakeStaticDamageEnvelope(
                root.players[0], root.players[1], root.position);
            if (envelope.enemyHp <= 0) return 0;
            if (turns <= 0) return 1;
            for (int t = 1; t <= turns; ++t) {
                if (envelope.DamageUpperAt(t) >= envelope.enemyHp) return t;
            }
            return turns + 1;
        }

        [[nodiscard]] bool StaticOptimisticKillPossibleWithinBattle(
            const BattleEmulator::SearchState &root, const int maxTurns) {
            const int turns = std::clamp(maxTurns, 0, kMaxPlanTurns);
            const StaticDamageEnvelope envelope = MakeStaticDamageEnvelope(
                root.players[0], root.players[1], root.position);
            if (envelope.enemyHp <= 0) return true;
            if (turns <= 0) return false;
            // The proof only needs the one-way question at its exact remaining
            // horizon. Do not scan 1..turns merely to compute an unused earliest
            // optimistic turn. If even this overestimate misses, the real branch is
            // impossible inside the horizon.
            return envelope.DamageUpperAt(turns) >= envelope.enemyHp;
        }
    } // namespace
    ...
    namespace {
        ...
        private:
            ...
            [[nodiscard]] RNGFLOW_FORCE_INLINE

            static std::uint16_t PackBound(
            ...
            static OptimisticBoundResult UnpackBound(
            ...
            static OptimisticBoundResult MergeBound(
            ...
            static std::uint64_t LeafHash(std::uint64_t x) noexcept {
                x ^= x >> 30;
                x *= UINT64_C(0xbf58476d1ce4e5b9);
                x ^= x >> 27;
                x *= UINT64_C(0x94d049bb133111eb);
    ...
    namespace {
        ...
        [[nodiscard]] std::size_t BuildBattleProofCandidates(
            ...
            // eight supported commands, but avoid eight cross-TU calls for every exact
            // frontier state. No legal command is removed.
        ...
        bool FindBattleWitnessDfs(
            ...
            if (!StaticOptimisticKillPossibleWithinBattle(state, remainingTurns)) {
                return false;
            }
            ...
            for (std::size_t i = 0; i < actionCount; ++i) {
                ...
                const int childOptimisticTurns =
                        StaticOptimisticKillTurnsBattle(child, remainingTurns - 1);
