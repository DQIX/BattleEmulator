...
namespace rngflow {
    ...
    ExactKillDecisionResult DiagnoseNoKillBattleExact(
        ...
        const auto started = std::chrono::steady_clock::now();
        ExactKillDecisionResult result =
            DecideBattleExactImpl(root, maxTurns, timeLimitMs, false, true, true);
        if (!result.complete || result.killReachable ||
            result.closestRejectedDepth < 0 ||
            result.diagnosticActionCount != result.closestRejectedDepth) {
            return result;
        }

        BattleEmulator::SearchState rejected = root;
        for (int i = 0; i < result.diagnosticActionCount; ++i) {
            BattleEmulator::SearchState child{};
            if (!BattleEmulator::StepSearchState(
                    rejected, {result.diagnosticActions[i]}, &child, false)) {
                result.complete = false;
                return result;
            }
            rejected = child;
        ...
        const int turns = std::clamp(maxTurns, 0, kMaxPlanTurns);
        const int tailTurns = turns - result.diagnosticActionCount;
        if (tailTurns <= 0) {
            result.diagnosticFinalEnemyHp = rejected.players[1].hp;
            result.diagnosticFinalHeroHp = rejected.players[0].hp;
            result.diagnosticFinalPosition = rejected.position;
            return result;
        }
        ...
        struct ClosestTail {
            bool found = false;
            int enemyHp = std::numeric_limits<int>::max();
            int heroHp = -1;
            int position = -1;
            int actionCount = 0;
            std::array<int, kMaxPlanTurns> actions{};
        } best;
        std::array<int, kMaxPlanTurns> tail{};
        std::uint64_t tailGenerated = 0;

        const auto searchTail = [&](auto &&self,
                                    const BattleEmulator::SearchState &state,
                                    const int remaining,
                                    const int depth) -> void {
            if ((tailGenerated & UINT64_C(0x0fff)) == 0 &&
                std::chrono::steady_clock::now() >= deadline) {
                return;
            }
            if (remaining <= 0) {
                if (state.players[0].hp <= 0 || state.players[1].hp <= 0) return;
                if (!best.found || state.players[1].hp < best.enemyHp ||
                    (state.players[1].hp == best.enemyHp && state.players[0].hp > best.heroHp)) {
                    best.found = true;
                    best.enemyHp = state.players[1].hp;
                    best.heroHp = state.players[0].hp;
                    best.position = state.position;
                    best.actionCount = depth;
                    std::copy_n(tail.begin(), depth, best.actions.begin());
                }
                return;
            }
            ...
            for (std::size_t i = 0; i < count; ++i) {
                BattleEmulator::SearchState child{};
                if (!BattleEmulator::StepSearchState(
                        state, {actions[i]}, &child, remaining == 1)) {
                    result.complete = false;
                    return;
                }
                ++tailGenerated;
                if (child.players[0].hp <= 0 || child.players[1].hp <= 0) continue;
                tail[depth] = actions[i];
                self(self, child, remaining - 1, depth + 1);
                if (!result.complete || std::chrono::steady_clock::now() >= deadline) return;
            }
        };
        searchTail(searchTail, rejected, tailTurns, 0);
        if (std::chrono::steady_clock::now() >= deadline) {
            result.complete = false;
            return result;
        }
        if (!best.found || best.actionCount != tailTurns) return result;

        const int prefixCount = result.diagnosticActionCount;
        for (int i = 0; i < best.actionCount; ++i) {
            result.diagnosticActions[prefixCount + i] = best.actions[i];
        }
        result.diagnosticActionCount = prefixCount + best.actionCount;
        result.diagnosticFinalEnemyHp = best.enemyHp;
        result.diagnosticFinalHeroHp = best.heroHp;
        result.diagnosticFinalPosition = best.position;
        return result;
    }
