**最短ターン数の証明仕様 — 近似による d(N−1)=偽 と、同じ規則で再実行する d(N)=真**

2026-09-05。`aaa.txt` にあるGPT5.6 solの指摘を受けた設計補足。
実装は行わない。本書は `d20-worldline-proof-spec.md` の第5・10・14・15・17節を具体化する。

局所検査から全到達遷移の被覆を導く帰納法と、量子化後の否定証明・最短性の数学的根拠は
[数学的証明](dN-mathematical-proof.md) にまとめる。新しい必須機能は追加せず、PoCの受理条件を証明する。

同日追記。現行ソースを調査し、Invariantの責任と領域の形式を第2・4・13〜16節で固定した。
初版のInvariantを任意の関数として実装してはならない。初版ではkernelが作る固定形式の外包だけを使う。
目標は、同じ入力・規則について勝利手順による `d(N)=真` と、外側近似による `d(N-1)=偽` を組にすること。
数百ms〜15秒は生成・検査・再実行を含む実測の目標であり、本書には特定入力の証明書や達成時間はまだない。
以下のv1の制限は、先行する本体仕様の一般的な記述より優先する。
v1のPoCは `yo2_be` のままとする。利用者が指定した前半系2例の調査結果と拡張条件を第17・18節に追記した。
前半系の約17ブランチ全体を調査済みとはしない。後半系への適用は対象外である。

2026-09-06追記。`aaaa.txt` の指摘に従い、乱数位置ごとの `Partition[p]` とSupportの全層再構築、
RuleProgramの登録時の有限実行性検査を明記する。第3節には責務を分けた実装案も示す。
規則を変更不可のversionで識別する方針、健全性の定理、外側近似の方向は変更しない。
同日追記。開始列Zだけを受けて将来の勝利列Wを探す入口を本体仕様第13節に追加する。
本書第16節にprefixとsuffixの接続検査、三値の返却条件、探索も含めた予算と評価条件を定める。

最大の難所は、DPへ渡す一ターンの遷移を、正しい領域と更新式に分解することにある。
次に難しいのが、それを粗く近似した際の架空の接続を、少数の分割で除けるかである。
DPの辺評価回数だけを根拠に、全体が数百ms〜15秒に収まるとは見積もれない。

特に、検査器が生成済みの辺だけを検査しても、生成器が落とした枝を検出できない。
以下では、**検査器が次に調べる命令と入力領域を自分で決める**方式にする。

**1. 信用する部分を固定する**

証明生成器から渡された命令列を、無条件に戦闘の定義として受け取ってはいけない。
検査器側に、対象戦闘の一ターンの規則を固定した `RuleProgram` を置く。
各命令は固定された位置 `pc` を持ち、分岐先、呼び出す関数、更新先もこの規則で決まる。

現行のC++には、この仕様の `RuleProgram` の型・登録器・検査器はまだ定義されていない。
第3節の模擬コードは将来の実装形式を示すもので、既存APIを呼べば既に動くという意味ではない。
「固定」の単位は命令列だけでなく、数値設定・対応profile・native関数・正確な再実行との対応を含む。
その内容を変更不可の `rule_id` に結びつける登録手順は第16節に定める。

信用する基盤は次の三つ。

1. 指定された戦闘規則に対応する `RuleProgram`。
2. 資源に依存しない乱数・ダメージ計算の限定された既存関数。
3. 整数式、区間、命令順序、証明記録を検査する小さい検査器。

探索・価格の選択・セル分割の選択・証明生成器・生成済みの辺一覧は、この信用の基盤に含めない。

ただし、`RuleProgram` が現C++と利用者の指定した修正規則に対応することは、別の検証対象である。
行番号や関数名を書いただけでは、その対応は証明されない。
元の比較・更新箇所との対応表を作り、分岐順序も含めて監査する必要がある。
本案はC++全体の意味を自動証明するコンパイラを要求しない。
その代わり、この対応を未検証のまま「現BattleEmulator全体まで形式検証済み」とは呼ばない。

対象モデルには、少なくとも以下の規則の選択を明記する。

```text
HEALの選択条件：MP>=2
CRACKの選択条件：MP>=3
眠り・麻痺中のFLEE禁止、および行動実行時の再判定
敵先攻時の状態異常と、その後の味方行動
カウンターによる敵HP更新と、その後の味方行動
v1では初期状態が眠っておらず、対象行動が眠りを書き込まないこと
```

FLEEの旧skipTurnを、そのままこの規則として取り込まない。
未定義の眠りの動作も推測で埋めない。未対応なら、検証済みの上限で覆うか未判定とする。
v1では眠りを含む入力を未対応とする。眠りなしを保証できる対象は第13節に限定する。

**2. 記号実行器が扱う型を小さく限定する**

四つの入力資源を `e,a,m,i` とする。
途中の資源式は、**各資源自身の入力座標**について次のどちらかだけを扱う。

```text
E_expr = e + k / k
A_expr = a + k / k
M_expr = m + k / k
I_expr = i + k / k
k: 検査された符号付き64bit整数。上の / は選択肢の区切り。
```

`E_expr=a+k`、`E_expr=2*e+k`、座標間の大小比較、任意の述語コールバックは受理しない。
固定ダメージを引くこと、固定回復量を足すこと、clampの後に定数にすることを扱う。
ダメージ自体は、必要な制御条件を分割してから既存の限定関数で具体的に計算する。

一個の領域の形式を次に固定する。

```text
Box = [le,ue] × [la,ua] × [lm,um] × [li,ui]
      × mask(Charge) × mask(Paralysis) × mask(Acro)
      × mask(Rage) × mask(Inactive) × mask(Camera)

資源区間は両端を含む整数区間。
modeの値とbit番号は第13節の固定enum。未定義bitは拒否する。
p、seed、選択コマンド、経過ターンtはBoxの外の具体値。
```

制御式は `Const(value)` または `Map(j, table)` のみ。
後者は同じ制御座標jの入力enum値を、小さい定数表で途中値へ写すものとする。
表の入力は最大8値なので、合成・像・逆像はその8値以下を調べればよい。
別座標に依存する更新は、その条件で先に分割してから定数代入等を行う。
途中の麻痺タイマー-3等は規則のpcで定めた一時値として扱い、境界enumには追加しない。
配列参照やnative関数の引数が未確定なら、その使用箇所で必要な値に分割する。
制御状態全体の直積列挙は行わない。

セル分割木で認める原子条件も、次の二つだけとする。

```text
入力資源X <= 整数k
入力制御j ∈ 固定enumのbitmask
```

規則の比較は現在の式を代入してこの形に戻す。
`X<k` は `X<=k-1`、等値は二つの閾値分岐で表す。境界計算もオーバーフローを検査する。
`2*E<456` のような固定正整数倍の比較は、対応を監査した整数閾値へ変換する。
論理積・論理和は規則の短絡評価順序に従う複数の分岐pcにし、任意式ソルバーを呼ばない。

kernelに必要な領域操作は以下で尽きる。

| 操作 | 実装する検査 |
| --- | --- |
| 空判定 | いずれかの区間で下端>上端、またはいずれかのmaskが0 |
| 共通部分 | 区間は下端のmax・上端のmin、maskはAND |
| 包含 | 全区間の端点包含、全maskで `source & ~target == 0` |
| 閾値の真偽分割 | 該当する一区間だけを `<=k` と `>=k+1` に制限 |
| modeの真偽分割 | 対象maskとのANDと、その型の範囲内での補集合とのAND |
| 更新後の像 | 自座標の平行移動・定数、または小さい写像表の像 |
| 出力セルの逆像 | 出力の分割木をたどり、資源式への代入または写像表の逆像で入力を制限 |

各RNG位置pは独立した分割木 `Partition[p]` を持ち、その一葉をlocal cellと呼ぶ。
各local cellはBaseBoxを上の原子条件で分割した一個のBoxになる。
不連続な集合は複数の葉・証明枝で表す。任意のBox和集合、DNF、多面体、DBM、SMTはv1に入れない。
凸包で二つの枝を通常葉へ合併することもしない。合併するなら、全体を覆う補完として検査する。
葉数だけでなく、これらの領域操作・写像表の処理・出力木の走査も第10節のVに数える。

```text
PredicatePartition = 有限な二分木（内部ノードは許可述語と真偽の二子、葉はlocal_cell_id）
PartitionFamily = frozen {
    partition_version,
    trees: map<RngPosition, PredicatePartition>
}
partitions: PartitionFamily                 # Partition[p] == partitions[p] == partitions.trees[p]
CellKey = (p: RngPosition, local_cell_id: LocalCellId)
Support[t]: set<CellKey>
Cell(partitions[p], local_cell_id): Box
project(alpha(S), partitions[S.p]): LocalCellId  # alpha(S)を含む唯一の葉
```

位置集合 `P_set={p∈Z | p0<=p<=p0+H*Rmax}` の各pについて木を定義し、各木がBaseBox全体を重複なく覆い、
`leaf_count(Partition[p])<=K` であることをkernelが検査する。`P=|P_set|` とする。
擬似コードの `integer_range_inclusive(lo,hi)` は両端を含む全整数の有限集合を表し、forはその全要素を列挙する。
`lo==hi` の場合は一つの位置を含む。
木の循環・未解決参照・重複local cell idを拒否し、各分割の真偽二子が非空で元のBoxを覆うことも確認する。
従って各木のノード数はleaf数Lに対して `2L-1<=2K-1` であり、木の保存量もO(PK)に収まる。
一つのpartition_versionの間は、同じpを別の層tで使っても同じ木を参照する。
`p=100` に `E<=227` を追加する操作は `Partition[100]` だけをrefineする。
他の位置へ自動伝播させず、必要なら生成器が位置を指定して個別に追加する。
local cell id単独や、別位置で同じ番号の葉を、同一セルとして照合してはならない。

出力の位置がp'なら、必ず `classify_output(output, partitions[p'])` で出力先を求める。
詳細葉ではこの木の述語の逆像で入力を分け、全出力先 `(p',local_cell_id')` を得る。
補完がp'の区間を持つ場合は、区間内の各p'の木の全葉を含める（第15節）。入力位置pの木で代用しない。
分割集合とその世代に依存する記録の寿命、未定義位置の拒否は第14節に定める。

この形式では資源式が自座標だけに依存するため、詳細葉の資源の像も直積のままである。
重みの最大値は `e=ue, a=la, m=lm, i=li` で計算できる。
これは式の係数の符号から得られる厳密な最大値であり、代表状態による試行ではない。

検査中の環境は概念的に

```text
Frame = (pc, call_stack, RNG_position, selected_command,
         control_expressions, E_expr, A_expr, M_expr, I_expr,
         transient_slots, action_progress, input_domain)
```

である。`action_progress` は、味方・敵の行動が未実行か実行済みか、
資源消費まで進んだか等を規則上の実行位置から決める情報である。
生成器が「未実行」と自由に宣言する情報ではない。
transient_slotsには準備済みコマンド・実行行動・カメラへ渡す行動列等を型付きで保持する。
これらは一ターン内部の作業値であり、ターン境界のmodeやDPの添字には追加しない。

実状態の保存方式は可逆なビットパックとする。`uint64_t[3]`（24byte）は設計案であり、現行C++の実装済みの型ではない。
`Frame` は局所証明を読むための作業領域であり、到達した全個体の保存ではない。
この作業領域や証明記録の容量は、実状態の格納領域とは別に計上する。

**3. 検査可能な命令と証明記録**

最小構成は次の種類で足りる。

| 規則側の命令 | 検査器が行うこと |
| --- | --- |
| 定数代入・加減算 | 現在の式から更新式を再計算。オーバーフローを検査 |
| 資源または制御の比較 | 真・偽の入力領域を再計算し、必要な両方の子へ進む |
| 固定有限集合のswitch | 全該当値の領域と分岐先を再計算。省略された値を検出 |
| 乱数読み出し | seedと現在位置から値を再計算し、演算形式と位置の進行を確認 |
| 固定個数の乱数読み飛ばし | 規則に書かれた個数だけ位置を進める |
| 限定された計算関数 | 規則側が指定する関数を、環境から決まる引数で再実行 |
| ジャンプ・呼び出し・復帰 | 規則側の次pcとスタックを確認 |
| 正常終了・成功・失敗 | 到達すべき終了位置、生死、出力式を確認 |

`COMPLETE` は検証済み上限で残り実行を覆う**証明ノード**であり、RuleProgramの命令ではない。
規則側の正常終了は `FINISH`、subroutineの復帰は `RETURN` とする。COMPLETEを規則の終了性の根拠にしない。

既存関数を一個の命令として扱えるのは、渡す引数が具体的に決まり、
隠れた資源読み出し・書き込みがない場合に限る。
例えば固定atk・defと乱数位置からダメージを計算する関数では、値と消費後位置を再計算できる。
`callAttackFun` 全体、`ProcessRage`、`process7A8` を資源に無関係な黒箱として扱ってはならない。
その中にある資源比較、状態変更、追加乱数消費を命令列に含める必要がある。

小さい固定回数のループは登録前または登録時にunrollし、一段のカウンターは非再帰の呼出しにする。
以下の有限実行性検査を登録の必須条件とし、規則上の比較・更新・乱数の順序を保存する。
一般的なSMT処理系や、任意のC++を解析する仕組みは要求しない。

**RuleProgramの模擬定義と、現ソースとの対応例。**
規則側は次のような読み取り専用データとする。証明書側のSTEP・SPLIT等とは別の型である。

```text
RuleProgram = frozen {
    instruction_format_version,
    entry_pc,
    routines: [Routine(id, kind=TURN|SUBROUTINE, entry_pc,
                       typed_parameters, typed_result, declared_reads, declared_writes,
                       instructions)],
    source_correspondence,       # 元の比較・更新・呼出しとpcの対応
    explicit_rule_changes       # 選択合法性、FLEE実行時ゲート等
}
Pc = (routine_id, instruction_index)
Instruction = tagged_union(opcode, typed_operands, prescribed_successors)

Instructionのopcodeは本節の命令表に限定する。
資源式・制御式・比較は第2節の型だけを使う。
乱数・native関数は登録済みの識別子で指定し、引数は確定した値だけを渡す。
CALL(callee_id, arguments, result_slot, return_pc) はこのprogram内の固定entryへ進む。
任意のC++関数への呼出しではない。return_pcは呼出し元routine内の固定pcである。
FINISHはTURNの一ターン終了、RETURNはSUBROUTINEからの復帰専用とする。
```

**RuleProgramを肥大化させない実装単位。**
RuleProgramは命令と対応表を保持する不変データに限定する。戦闘実行、登録、証明生成、
セル分割、DP、キャッシュ、再実行をメソッドとして集めたクラスにはしない。
次の名前は実装予定の責務を表すもので、今回ファイルやC++の型を追加する指示ではない。

| 実装単位 | 入力と出力・責任 |
| --- | --- |
| 規則作成用moduleとRuleAssembler | 小さいroutine定義を受け、固定ループ展開、型付きラベル解決、pcとソース対応表の生成を行う。未検査のRuleProgram案を出力する |
| RegistrationChecker（kernel） | 同じ不変な案の型・全case・CFG・call graph・native契約・profileを検査し、後述の静的上限を計算する。Assemblerの成功表示や手書き上限は信用しない |
| RuleRegistry / RuleBundle | 検査済みProgram、数値・native、profile、再実行対応とkernel算出の上限を一つの変更不可のrule_idへ束縛する（第16節） |
| SymbolicStepper / ProofVerifier（kernel） | RuleProgramとFrameから、次pc、更新、全非空子領域を決め、証明ノードを照合する。opcodeだけを解釈し、技名別の戦闘switchをもう一つ作らない |
| CellConnector / SupportBuilder（kernel） | 検査済みのガード・更新・補完をpartitionsへ接続し、全必要根を要求する。命令を変更せず、RuleProgramにセル番号や価格を埋め込まない |
| 証明生成器・価格DP | 許可形式の分割・展開・価格を提案し、検査後の辺を使う。登録規則、必要root、合法性を差し替えない |
| WitnessCandidateIterator（生成側） | 同じ検査済みグラフから有限予算で候補コマンド列を列挙する。本体仕様第13節のL・順序・重複管理を持ち、RuleProgramやFALSE用modelを変更しない |
| ExactReplayAdapter | RuleBundleに固定した生状態の再実行入口へつなぐ。FLEE等の明示的な修正を含む対応を監査する。記号実行の代表点で代用しない |

規則作成用moduleは、次の単位でroutineを提供する。一つのroutineが行動種別の全switchを抱える必要はない。
moduleの分割と、後述するroutine単位のcall graph検査は別である。

| moduleの責務 | 現ソースとの対応と所有する処理 |
| --- | --- |
| TurnFlow | Mainの一ターン分。必殺減算、defence初期化、先攻RNG、選択済みコマンドの準備、二つの行動枠と枠前の生死判定、ターン末尾への順序だけを組み立てる |
| AllyActions / EnemyActions | callAttackFunをcaseごとのroutineへ分解する。敵選択はProcessEnemyRandomAction2BとMainの置換・追加消費を含む。case内の支払いや副作用は元の位置に置く |
| StatusTiming | 味方枠の麻痺減算・解除・inactive処理、味方行動後のアクロ失効、敵行動後の怒り減算を別entryにする。所有者・生存条件・減算pcを固定し、一括したターン末尾処理へ移さない |
| ReactiveEffects | ACROBATSTAR_KAIHI、COUNTER、ProcessRage、process7A8を別routineにする。カウンター側の敵HP減算もここから明示的に呼ぶ |
| ResourceUpdates | Player::reduceHp / healを、型付き資源の減算・加算とclampへ展開する。Mainの通常結果反映と、case内のHP_HOOVER回復・COUNTER減算を区別する |
| PresentationTail | 行動列とCameraから決まるカメラ処理を登録する。Mainのactions配列はカメラへの入力なので、ログとして削除しない。実体との対応は既存の第13節の監査対象である |

Mainの複数ターンのfor、Gene解釈、ログ出力、観測による早期returnは、一ターンの命令列の外へ分ける。
開始ターンと観測・行動制約はProblemと再実行対応に残し、Mainのbool戻り値をそのまま勝敗にはしない。
一ターンの通常の継続境界は規定の末尾・カメラ処理後である。尾部を省く入口を使うなら、その対応も別途固定する。

例えばMainの行動順序は、次の構成用記法で表せる。各CALL先は後続の処理へ勝手に飛ばずRETURNする。
二枠をunrollし、同じslot routineを二回CALLしてもよい。生死判定・break相当の分岐は各元位置に残す。

```text
TURN_ENTRY -> BEGIN_TURN -> PREPARE_SELECTED_COMMAND
  -> initiativeの固定分岐
     ally先攻:  SLOT_ALLY -> SLOT_ENEMY
     enemy先攻: SLOT_ENEMY -> SLOT_ALLY
  -> TURN_TAIL -> CAMERA_TAIL -> FINISH

SLOT_ENEMY:
    枠入口の生死判定で、終了済みならRETURN
    action = CALL(SELECT_ENEMY)
    SWITCH(action) -> 各caseの固定CALL                    # 間接CALLは禁止
    actionsへ実行したactionを追加
    CALL(APPLY_ENEMY_RESULT, returned_damage)             # 味方HPへ反映
    CALL(ENEMY_POST_ACTION)                              # 生存判定・消費・怒り減算
    RETURN

ENEMY_ATTACK / HP_HOOVER -> アクロ判定の各条件からACRO_DODGE / COUNTER / 通常被弾へ分岐
COUNTER -> 規定順のRNG・会心・回避・ダメージ計算
  回避あり: RETURN(value=0)
  回避なし: CALL(RAGE_CROSSING) -> CALL(REDUCE_ENEMY_HP) -> RETURN(value=0)
```

PREPARE_SELECTED_COMMANDでは、選択済みコマンドからMainのDEFENCE差替とdefence倍率設定を元の位置で行う。
選択したcommand、準備後のcommand、状態異常で差し替わる実行actionは区別する。HPによる未指定行動のフォールバックは使わない。
現C++の `callAttackFun(COUNTER, ...)` は同じC++関数を呼ぶが、IRでは別routineへの固定CALLである。
COUNTERから元の敵攻撃caseへ呼び戻す辺を作らないため、C++の見かけ上の自己呼出しをIRの再帰にしない。
カウンターが返した0はMain側の味方被ダメージだけに使い、既に減った敵HPを二重に減算したり戻したりしない。
同じ補助routineを共有する場合も、引数・効果・乱数順序が一致する部分に限る。

Frame内の資源式・mode式・RNG位置はkernelが所有する。routineの引数と戻り値は型付きslotで受け渡し、
局所slotはCALLごとに分ける。生のPlayerポインタ、任意の参照、文字列によるフィールド検索は命令の引数にしない。
HPの保存式には由来の資源座標を保持し、型の違う資源への代入を許さない。
共有状態へのread/writeは宣言された型付きフィールドに限定し、登録検査が呼出し先を含めて再計算する。
未初期化slot、引数・戻り値の型不一致、宣言外の書込みを拒否する。補完の行動済み情報もpcとこの効果から検査する。

approved nativeの窓口は `invoke(native_id, concrete_arguments, bounded_rng_cursor)` に限定する。
戻り値と実際のRNG増分をkernelが得て検査し、nativeへPlayer・Frame・NowStateを直接渡さない。
契約には固定実体、引数の許容域、戻り値型、終了性と有限な内部work上限、RNG消費上限を登録する。
I/O、待機、VMへの再入、隠れた状態変更・RNG消費は認めない。C++実体が契約を満たすことの監査は第1節の責任であり、
kernelは名前だけから終了性を推論せず、監査済み契約への登録照合と引数・結果・増分検査を行う。
FUN_0207564cやtypeC/typeDはこの限定窓口の候補だが、内部LCGを含む契約が揃って初めて登録できる。
新しい技の追加はcase routine・必要な対応表・profile・補完表の更新で行い、原則としてkernelのopcode解釈を増やさない。
第17・18節の横展開では、登録profileに従って資源座標・有限enum・行動枠を拡張する前提を維持する。
その拡張後も自座標の式・有限modeという形式で表せない技は、この構造へ無理に押し込まず未対応とする。

**有限実行性の登録検査（v1必須）。**
kernelは汎用のループ停止性解析を作らず、次の強い構造制限を使う。
手書きの「最大64消費」「再試行は有限」という宣言だけで受理しない。

1. 全routineの命令集合と通常CFGを有限にする。通常CFGはroutine内の次pc・分岐・jumpを辺とし、
   CALLではcalleeをその場で展開せず、規定のreturn_pcへの辺を置く。このCFGをトポロジカル順序で検査し、cycleを拒否する。
2. 固定回数ループは登録前または登録時に有限回unrollし、break・短絡評価・早期復帰の順序を保つ。
   元のMainの二枠、process7A8の最大9比較はその例である。複数ターンの反復はProgramの外のHで行う。
   再選択も、登録した有限表と条件から確認した上限で展開する。上限後の再試行枝を黙って削除してはならない。
3. 命令pc、全jump先、各switchの全case、CALL先・引数型・return_pcを解決する。
   未解決jump、未登録call target、間接CALL、routineをまたぐjumpを拒否する。
4. routine間のCALL辺から作る有限なcall graphもacyclicとする。自己再帰・相互再帰をともに拒否する。
   「一回だけ再帰する」という実行時フラグに頼らず、COUNTERの例のようにentryを分ける。
5. 全entryのすべての制御経路が、TURNならFINISH、SUBROUTINEならRETURNへ終わることを検査する。
   DAGの末端に別の命令、暗黙のfallthrough、未処理caseを残さない。到達不能という生成器の申告で検査を省かない。
6. 最大call depthと、全entryからそのFINISHまたはRETURNまでのmaximum instruction stepsを静的に計算する。
   call depthは未復帰のCALLフレーム数（TURN入口で0）。命令stepにはCALL・RETURN・FINISHも各1を数える。
7. 同じ有限CFGと登録native契約から、RNG read / skip / approved nativeの最大消費数を計算する。
   読み飛ばし個数は非負の固定値にし、状態依存なら先に有限caseへ展開する。位置の後退を許さない。
   nativeの有限終了・work・RNG消費上限が未登録、または許容引数域を保証できない場合も拒否する。
8. 一つでも検査できない案、静的上限の計算がオーバーフローする案、登録用の容量制限を超える案は登録拒否する。
   証明時にCOMPLETEを置く予定であることを、これらの条件の免除には使わない。

上限の算出は次の小さい後ろ向きDPで足りる。call graphでcalleeから先に処理し、
routine内は通常CFGの逆トポロジカル順に処理する。分岐条件による経路の実現可能性の解析は不要で、
構文上の全後続のmaxは保守的な上限になる。`S_f,R_f,D_f` はroutine fのentryの算出結果とする。

```text
pcがFINISHまたはRETURN:
    S(pc)=1; R(pc)=0; D(pc)=0
pcがCALL(g, ..., return_pc=k):
    S(pc)=1 + S_g + S(k)
    R(pc)=R_g + R(k)
    D(pc)=max(1 + D_g, D(k))
それ以外（後続集合Succは非空）:
    S(pc)=1 + max(S(s) for s in Succ)
    R(pc)=rng_cost_upper(pc) + max(R(s) for s in Succ)
    D(pc)=max(D(s) for s in Succ)

rng_cost_upper: readは登録RNGの消費数、skipは指定数、nativeは契約の上限、その他は0
maximum_instruction_steps = S_TURN
maximum_call_depth = D_TURN
Rmax = R_TURN
```

nativeを一つの命令stepと数えても、内部の計算費が1になるわけではない。
nativeの内部workは監査済み上限で別に加算し、最大命令step数だけで時間を見積もらない。
以上の結果をkernel由来の登録情報として保持し、証明書から別のS・D・Rへ置換させない。

中断時のstackもkernelがCALL/RETURNから作る。現在pcからそのroutineの出口までの `R(pc)` に、
stackに積まれた全復帰先kの `R(k)` を足したものが `Rremaining(pc, stack)` である。
CALL先の消費は各Rに既に含まれるので二重加算しない。stackが空なら `Rremaining=R(pc)`。
有限CFGと有限call graphを検査済みだから、この和と残り命令step数を有限時間で計算できる。
Frameのstack長がmaximum_call_depthを超える場合、不正な復帰先、TURNでのRETURNを拒否する。
検査器の実装は明示的なwork stackを使い、説明用verify_nodeの再帰をそのままホストのcall stackへ載せない。

一ターンの具体的な一経路の長さと、全領域のproof workは別である。
命令ごとの有限分岐数（引数を確定する有限mode分割も含む）の上界を `Fsplit>=1` とすると、
詳細展開の木は保守的に `sum(Fsplit^i, i=0..maximum_instruction_steps)` 個の命令訪問で抑えられる。
各訪問の領域演算・nativeのworkと、末端での出力木の最大 `2K-1` ノードの分類費も掛けて評価する。
この上界は巨大になりうる。評価は予算超過を表す値で飽和させてよく、木を事前生成する指示ではない。
実際には第10・16節の累積V・byte・時間を優先し、許可pcでの補完までの予約ができなければUNKNOWNとする。
証明側の循環参照・無意味な追加stepも拒否し、有限実行性から小さい証明や15秒達成を推論しない。

以下の局所例のラベルは固定pcの説明用表記である。複数操作を含む行は順番を保存した命令列へ展開する。
`max` によるclampも比較・真偽の更新へ展開し、全非空領域を検査する。
一ターン全体の登録には全呼出し先と全caseが必要であり、この抜粋だけを完成したRuleProgramとして登録しない。

`ProcessRage` は、ダメージDが既に具体値に確定した後、敵HPを書き換える前に呼ばれる。
敵最大HP456、非負のD、整数安全性を検査した領域で、次のように表せる。

```text
R00: before = current(E)                 # 自座標の式の保存。Eは更新しない。
R01: after = max(0, before-D)
R02: if after <= 227:
    R03: if before >= 228:
        R04: if Rage == OFF:
            R05: SkipRng(1)
            R06: r = ReadRng(INT_RANGE, 2, 4)
            R07: Rage = ON(r)
        else:
            R08: SkipRng(1)
    else:
        R09: if after <= 113:
            R10: if before >= 114:
                R11: if Rage == OFF: SkipRng(2)
                else:               SkipRng(1)
R12: RETURN
```

`after<=227` と `after<=113` は、現ソースの `hp_after*2<456` と `hp_after*4<456` に対応する。
R03の真側ではR09へ進まないため、両閾値を一度に越えても25%側の追加消費は発生しない。
R06〜R07は、生の `rage=true`、残り値抽選、`rageTurns` 代入を一つのON更新へmode化した表記である。
乱数関数はRageを読まず、この代入途中で中断を認めない。生の再実行では元の代入順を保持する。

カウンターの呼出し元は、非回避側で次の順序になる。

```text
COUNTER_HIT: CALL(R00, D)
COUNTER_HP:  E = max(0, E-D)             # Player::reduceHpの減算とclamp
COUNTER_RET: RETURN(value=0)             # 敵から味方への戻りダメージは0
```

戻り値0を「敵HPも変化しない」と解釈しない。次の味方行動や終端検査は更新後のEを読む。
通常の味方攻撃では、怒り判定後にも追加乱数・必殺判定があり、最後に `Main` が敵HPを減らす。
`process7A8` の `hp>D` と、味方攻撃側の `hp-D>=0` も別の比較pcとして残す。
HEALのMP2消費はcaseの後部、CRACKのMP3消費はcaseの冒頭にあり、共通の入口へ移動させない。

FLEEの修正は、味方行動枠の入口で次のように表す。眠りを含む入力は先にPoCのprofile検査で拒否する。

```text
ALLY_GATE: action = prepared_command
    if action == FLEE_ALLY and Paralysis == CLEAR and Inactive == false:
        goto ALLY_SLOT_DONE             # 行動可能なFLEEの既存skip部分
    goto ALLY_STATUS

ALLY_STATUS:
    if Paralysis != CLEAR:
        action = CALL(PARALYSIS_GATE)   # 減算・解除抽選。戻り値はPARALYSISかCURE_PARALYSIS
    else:
        SkipRng(1)
    if Inactive:
        Inactive = false
        if action != PARALYSIS and action != CURE_PARALYSIS:
            action = INACTIVE_ALLY
    SWITCH(action) -> 各ALLY_CASEの固定CALL、戻り値をDへ格納
    actionsへ実行したactionを追加
    CALL(APPLY_ALLY_RESULT, action, D)  # Mainの味方回復または敵HP減算。
    CALL(ALLY_POST_ACTION)              # 両者生存時の消費とアクロ減算・失効。
    goto ALLY_SLOT_DONE

ALLY_SLOT_DONE: RETURN                  # TurnFlowの規定の次枠・ターン末尾へ復帰
```

ここでのCALL先は元の処理を分解して登録する命令列であり、状態を隠して読むnative命令ではない。
敵先攻で麻痺したFLEEは解除に成功してもCURE_PARALYSISの行動になり、選択時のFLEEへ戻さない。
行動可能なFLEEのskip部分は、味方case・味方行動配列への追加・その枠のアクロ減算を通らない。
敵行動や規定のターン末尾・カメラまでskipする意味ではなく、各行動枠前の生死判定も元の位置に置く。
このゲートは第13節の明示的な修正規則であり、現在の `Main` に既に実装されているとは扱わない。

証明記録のノードは、例えば次を持つ。

```text
node_id
expected_pc
kind = STEP / SPLIT / SWITCH / FINISH / COMPLETE
claim_payload
child_ids
```

入力領域と途中の式は、原則として親から検査器が計算する。
ノードにそれらの値を保存する場合も、計算した値と完全一致するか確認する。
ノードごとに全状態を重複保存する必要はない。

**4. 根と子を、生成器に決めさせない**

検査器は各必要な `(経過ターンt, 乱数位置p, local_cell_id=ell)` と8行動について、自分で次を計算する。
DPの添字qは常に組 `(p,ell)` であり、ell単独ではない。

\[
D_{t,p,\ell,b}=\left.\operatorname{Envelope}_t\right|_p
\cap\operatorname{Cell}(\operatorname{Partition}[p],\ell)
\cap\operatorname{Selectable}(b).
\]

根領域が空でなければ、その行動の証明木が必須である。
証明書に行動の根が載っていないことを、「その行動は不可能」と解釈しない。
根領域が空という主張も、検査器自身が確認する。

旧記法の `Invariant` は第14節の `Envelope_t` に置き換える。
これは入力と固定enumからkernelが組み立てるBoxとpの範囲であり、生成器が供給する関数ではない。
`Selectable` も固定する。HEALは `M>=2`、CRACKは `M>=3`、薬草は `I>=1`、
ACROは `Charge=ON(1..6)` かつ `Acro=OFF`、FLEEは `Paralysis=CLEAR`。
眠りなしは入力プロファイルが保証する。ATTACK・DRAGON・DEFENCEには追加の資源条件はない。
行動実行時の状態異常による差し替えは、根を消す条件ではなく規則内部の遷移である。

未到達セルを省略する場合は、被覆済みの辺から再計算した支持集合などの根拠が必要になる。
セルや行き先が表に存在しないというだけでは省略できない。
第14節に、この支持集合をkernelが前向きに作る手順を定める。

比較 `g` に出会ったら、検査器が

\[
D_T=D\cap g,\qquad D_F=D\cap\neg g
\]

を計算する。空でない側の子が欠けていたら不合格。
両方が非空なら、両方を検査しなければ根の検査は完了しない。

たとえば、この箇所の確定ダメージが16、入力敵HPが `[114,456]` なら、

```text
比較：E-16 < 228
真側：入力E ∈ [114,243]
偽側：入力E ∈ [244,456]
```

となる。これは説明用の局所例であり、特定seedの実行結果を報告したものではない。
偽側を省略した証明は拒否する。
真側を `[114,242]` にしてHP243を欠落させた証明も拒否する。

ただし、両側の和集合が親と一致するだけでは足りない。
**比較式そのものが、現在pcで規則が要求する比較でなければならない。**
生成器が本来の怒り判定を飛ばして、無関係な比較で領域を覆っても拒否する。

同様に、乱数記録に `p=123` と書かれていても、それを次の位置として信用しない。
現在の環境から得た位置が123なのかを確認し、そのseedの値を読み直す。
途中の読み飛ばし一個を欠落させれば、以降のpcまたは乱数位置で不一致になる。

**5. 検査手続き**

以下のt、Envelope、partitionsは、第14節の根検査が渡す検査コンテキストから取得する。
証明ノードに書かれたターン数・外包・別世代の分割集合へ置き換えない。

```text
verify_node(program, expected_frame, proof_node):
    assert proof_node.expected_pc == expected_frame.pc

    if proof_node.kind == COMPLETE:
        assert completion_lemma_is_allowed_here(expected_frame)
        summary = verify_completion_lemma(expected_frame, proof_node)
        assert summary has a checked output envelope, including terminal resources
        assert every continuing output lies in Envelope[t+1]
        emit_checked_summary(summary)
        return

    instruction = program.routines[expected_frame.pc.routine_id].instructions[expected_frame.pc.instruction_index]

    if instruction is deterministic update / RNG / approved native computation:
        next_frame = independently_execute(instruction, expected_frame)
        assert payload agrees with the recomputed values
        verify_node(program, next_frame, required_child(proof_node))

    else if instruction is JUMP / CALL / RETURN:
        next_frame = advance_control_and_checked_stack(program, instruction, expected_frame)
        assert payload agrees with the prescribed target, arguments and return value
        verify_node(program, next_frame, required_child(proof_node))

    else if instruction is branch:
        for each branch required by instruction:
            child_domain = exact_restriction(expected_frame.domain, branch.condition)
            if child_domain is not empty:
                child_frame = advance_to_prescribed_branch(expected_frame, branch, child_domain)
                verify_node(program, child_frame, required_child(proof_node, branch))

    else if instruction is finish:
        assert expected_frame.call_stack is empty and pc belongs to TURN
        assert claimed_resource_updates == expected_frame.resource_expressions
        assert claimed_terminal_kind agrees over the entire input domain
        outputs = exact_image(expected_frame)
        assert continuing outputs are contained in Envelope[t+1]
        assert terminal outputs have nonnegative HP, MP and item counts
        if outputs are continuing:
            p_out = outputs.RNG_position
            destinations = classify_output(outputs, partitions[p_out])
            assert claimed destinations cover every computed (p_out, local_cell_id)
            # 各出力葉の述語の逆像で根領域を制限する。分類中の省略は不可。
        recompute_weight_bounds_and_emit_all_checked_edges(expected_frame, partitions)

    else:
        reject
```

葉は、規則の終了位置へ到達した場合か、適用可能な補完補題を検査した場合だけ認める。
比較を処理する前に通常の終了葉を置くことはできない。

出力先セルの検査は、出力位置p'の `Partition[p']` の述語を更新式へ代入した逆像で行う。
複数位置を含む補完は、各p'の木に対して第15節の全行き先を構成する。
一個のセルに入ると主張するなら、入力領域全体でその述語が成立する必要がある。
複数セルへ分かれるなら、入力領域をさらに分割するか、全出力先を含む補完として扱う。
包含検査の前に出力を `Envelope[t+1]` と交差させてはならない。
継続出力が外へ出るなら、その証明書を拒否する。
成功・失敗終端は継続用Envelopeとは別に分類し、終端資源の非負と終了位置を検査する。
補完の適用箇所と形式は第15節のホワイトリストに限定する。

実装を小さくするため、最初は木として検査してよい。
共有ノードを許す場合は、循環参照を拒否し、参照先のpc・stack・環境・領域との適合も検査する。
同じノード番号というだけで別の途中状態へ証明を流用してはならない。

**6. この検査で、枝落ちを検出できる理由**

根の任意の具体的な入力状態を一つ選ぶ。
定数計算と乱数命令では、検査器が規則から結果を再計算するため、実行との対応が保たれる。
分岐命令では、その入力は検査器が作ったいずれかの非空領域に含まれ、その子は必ず存在する。
したがって、実行を進めても証明木から入力が消える箇所はない。

終了位置まで進めば、その入力の出力は検査済みの葉の式と出力先に含まれる。
途中で補完に進む場合も、検査済み補題がその入力からの残り実行を含む。
根がすべての合法入力を覆うので、各具体的遷移は検査済みの辺または補完に含まれる。

これで初めて、本体の第8節にある「全実遷移が被覆される」という仮定を満たせる。
検査済みの葉から辺を導出し、その辺についてDPの不等式を調べる。
生成器が別途提出した辺一覧だけを正本にしない。

この結論は、第1節で固定した規則と限定された計算関数に対するもの。
その規則自体の写し間違いまで、被覆木の検査だけで検出できるとは主張しない。

**7. セルを細分化しても、戦闘の記号実行を最初から繰り返さない**

計算を次の二つに分ける。

```text
規則の実行：ガード領域 D と、そこで成立する更新式 F を得る
セルへの接続：D と F を現在のセル分割に当てはめ、辺を得る
```

詳細な葉の記録は、固定seed・入力乱数位置・行動・規則・入力ガードに対応させる。
そこに価格や、現在のセル番号を埋め込まない。
セルへの接続と価格は、この記録から作る派生情報とする。

例えば入力位置pの元のlocal cell `C` を、同じ `Partition[p]` の `C1,C2` に分ける場合、既存葉 `(D,F)` は

\[
(D\cap C_1,F),\qquad(D\cap C_2,F)
\]

に制限できる。空の側だけ除く。
既に `D` 全体で更新式を検査しているので、その部分領域で乱数・ダメージを計算し直す必要はない。

出力位置p'の `Partition[p']` の出力先を `C1',C2'` に分ける場合も、

\[
D\cap F^{-1}(C'_j)
\]

で入力領域を分割する。
例えば `E'=E-16` と `E'<=227` なら、入力を `E<=243` で切るだけでよい。
clampや制御値の更新も、検査済みの式を使って逆像を計算する。

この再利用の根拠は単純である。
「Dの全点で成立する更新式」は「Dの部分集合の全点」でも成立する。
出力述語の逆像で切れば、各葉の出力先も正しく分類される。
この二つは新しい戦闘実行を必要としない。

補完葉には正確な残り更新式がないので、これを詳細葉と同じように使ってはいけない。
補完の対象領域を狭めることはできるが、詳細が必要になった部分だけは、
保存したpcと式から記号実行を再開する。
そこで得た詳細葉も、後の分割で再利用する。

「同じ部屋」「同じ乱数位置」だけを再利用条件にしてはならない。
規則、seed、開始pc、制御条件、資源式、証明された領域との包含が必要である。
規則がターン番号を読む場合は、その番号から決まる行動回数等の実行条件も一致させる。
第18節の奇偶で敵行動回数が変わる例では、同じpでも別の局所遷移になる。
保存する識別子は検査済み記録への添字でよく、実状態ハッシュ表は不要。

再利用のために古い証明やセルの写しを無制限に残すこともしない。
証明ノード・派生した辺・更新式の記録に、総byte数と総個数の上限を持たせる。
共有する記録と、現在の分割に依存する参照を分けて計上する。

`partitions[p]` をrefineしたら、Supportは新しいpartition family全体に対してS0から全層再構築する。
変更位置だけ木を差し替え、他位置の木を共有してもよいが、旧Supportの部分更新だけで済ませない。
再利用する詳細葉 `(D,F)` と、位置・local cell id・分割世代に依存する支持・根一覧・接続は別の寿命を持つ。
第14節の `rebuild_support` で新しい全出力先と全必要根を確定してから、DPを再計算する。
補完の詳細化で行き先集合が変わった場合も同じ手順を使い、旧DP・区間最大表・LP下界を引き継がない。

検査済みのガードと資源増減ベクトルは、非負の価格を変えても利用できる。
この検査を価格候補ごとに繰り返す必要はない。
最終検査でも、検査器自身が同じ実行内で確定した記録を使える。
生成器が付けた「検査済み」フラグを信用することとは区別する。

**8. 補完辺に「何でもできる残り一ターン」を毎回与えない**

本体にある

\[
W_{\rm top}=QD_{\max}+uA_{\max}
\]

は安全な粗い上限だが、攻撃を選んだ辺にも無料の回復を許してしまう。
最大経路がこれを繰り返す場合、価格の調整だけでは改善しにくい。
実装では、行動と実行位置から直ちに分かる制約を補完にも残す。

以下は現在の8行動、固定能力値に対する**一ターン全体の上限**である。
通常攻撃の最大64、DRAGON・CRACKの直接ダメージは保守的に最大34、
一回のカウンターは最大64という上限を使う。
能力値や行動集合を変えた場合は再検証する。

| 選択した行動 | 使える補完重みの上限 |
| --- | --- |
| ATTACK | `128 Q` |
| DRAGON | `98 Q` |
| DEFENCE、FLEE、ACRO | `64 Q` |
| CRACK | `max(64 Q, 98 Q - 3 v)` |
| HEAL | `64 Q + max(0, u*gH - 2 v)` |
| 薬草 | `64 Q + max(0, u*gI - w)` |

`gH` は一回のHEALによる味方HP増加の上限で、粗く最大HP65を使える。
`gI` は薬草による増加の上限で、現在の計算では39以下。
敵の与ダメージは味方HPの増加を大きくしないので、これらは安全な上限である。
敵の吸収回復も敵HPの正味の減少を大きくしない。

回復しない行動には、味方HPの正の増分を与えない。
一方、FLEEの直接ダメージが0でも、敵行動に対するアクロのカウンターは起こりうる。
一ターン全体の敵HP変化を一律0とせず、それが不可能と分かった箇所だけで0へ締める。
FLEEの辺自体は、重み0になっても消さない。

CRACK・HEAL・薬草を二つの場合に分けた理由は、行動不能や戦闘終了によって、
選んだコマンドが実行されない場合を含めるためである。

```text
HEALが実行された：HP増加<=gH、MP増分=-2
HEALが実行されなかった：HP増加<=0、MP増分=0
```

この二つの組を保つので、HP増加だけ得てMP消費を免れる補完にはならない。
CRACKも、直接ダメージを得る側ではMP3を必ず消費する。
全場合へMP消費を押し付けるのも、全場合で消費を0にするのも避ける。

これらは個体の優先順位ではなく、全該当遷移が満たす資源収支の補題である。
検査器が行動、進行状況、固定能力値から適用可能性を確認する。
表の値を、証明生成器が自由に小さく変更することはできない。

**9. 記号実行の途中で止める場合は、実行済みの収支を保持する**

入力を `S`、中断位置まで進んだ状態を `Sc=Fprefix(S)` とする。
残りのプログラムに対して使える重みの上限を `Uremaining` とすると、補完重みは

\[
\max_{S\in D}\{
Q(E(S)-E(S_c))+\Phi(S_c)-\Phi(S)
+U_{\rm remaining}(pc,S_c)
\}
\]

とする。既に払ったMP、既に受けたダメージ、既に起きたカウンターを消してはならない。
式の上限が必要なら、検証済みの区間・式で計算する。

例えば、

- 味方と敵の行動が終わり、カメラだけが残るなら、残りの資源増減はすべて0。
- 味方の行動が終わり、敵の行動だけが残るなら、残りでHEALや薬草は使えない。
  ただしアクロのカウンターが可能なら、敵HP減少の可能性は残す。
- HEALのMP2消費まで進んだ後に中断したなら、「HEAL未実行」の場合へ戻してMPを返してはならない。

残りの乱数位置・制御状態は、別途その実行位置からの可能な集合で覆う。
資源変化が0になったことから、乱数位置やカメラも不変とは推論しない。

この補完は、途中まで掘った情報を捨てて一ターン全体の最悪値に戻す方式より弱くならないように作れる。
全体の粗い上限も残し、両方が検証済みなら小さい方を使うこと自体は安全である。
ただし価格探索で本体の凸性を利用する場合は、採用する上限の表現に注意する。
第8節のような**固定された有限個の一次式の最大値**を標準の補完形式とする。
任意の上限同士の最小値を取る拡張では、同じ切除平面法がそのまま使えるとは仮定しない。

一個の補完記録が複数の一次式を持つ場合、その式の数も計算予算に含める。
辺のヘッダ一個に大量の式を隠して、辺数だけ小さく数えない。

**10. 証明木とDP用の辺は、別々に上限を持つ**

詳細な葉がJ個しかなくても、そこへ到達する途中の命令列や中断箇所の数はJとは限らない。
未展開部分をDPの一個の補完辺にまとめても、証明側では各中断領域の被覆を確認する必要がある。
共通祖先で使える一つの補題で覆う場合を除き、中断したpcと領域を勝手にまとめない。

制限する量を少なくとも次の四つに分ける。

```text
K：各RNG位置pのPartition[p]のleaf数の上限
J：CellKey=(p,local_cell_id)・行動あたりの詳細な辺の項数
C：CellKey・行動あたりの補完の一次式と行き先集合の項数
V：生成・再利用・独立検査を含む命令/証明ノードの総処理量
```

セル数は `sum_p leaf_count(Partition[p])<=PK`、DPの各層の評価対象は `8 P K (J+C)` 項以下とする。
検査記録の総byte数にも別の上限を設ける。
制限に達したら、適用可能な補完へ切り替えるか未判定とする。
証明ノードの途中を黙って省略してはならない。

一ターン内の経路長の上限は、第3節で登録時に計算するmaximum instruction stepsである。
これと最大call depth・native内部workから一経路の処理を見積もれるが、生成費が0だとは扱わない。
分岐の全子を検査するproof workには第3節の木の上界と累積Vを使い、Jだけから上限を推測しない。
分割・領域の制限・記号実行の再開を何回行ったかも合計して数える。

**11. 反復で、次に何を直すかを切り分ける**

最大経路が見つかったときの処理を分ける。

| 最大経路の問題 | 次の処理 |
| --- | --- |
| 粗い補完の未実行部分が上限を押し上げる | そのpcと領域だけを再開し、資源の消費・回復の組を保つ |
| 詳細辺同士が異なる実HPや制御状態を仮定してつながる | 食い違う比較の述語を追加し、既存の式を領域制限して接続し直す |
| 資源の収支の価格でまだ改善できる | 同じ検査済み辺で価格を再評価する |
| 正確に接続でき、正本で勝利する | TRUEを返す |

将来の回復を先借りして途中の死亡を免れるような抽象経路は、
総収支の一次式だけでは除けない場合がある。
生存に必要な `A>D` などを、必要な位置でセルに残す。
その述語を直前の更新式に沿って逆算すれば、前の位置で必要なHP条件も得られる。

全状態を探索してから述語を選ぶ必要はない。
一方、少数の述語で必ず十分になる保証もない。
小さいK・J・Cの範囲で証明の余裕が改善するかが、この方式の成否になる。

領域の分割や補完の改善で抽象グラフが変わったら、価格の候補は再利用できるが、
旧グラフから作ったLPの下界を新グラフの下界として使わない。
価格探索の制約は、新グラフに対して作り直す。

**12. 実装前後の判定基準をDPから移す**

実装担当者が最初に確かめるべきなのは、次の三点。

1. 一ターンの証明記録を、固定規則の命令順序に沿って生成・独立検査できるか。
2. 枝や乱数読みを一つ欠落させた記録を、本当に拒否できるか。
3. 小さい領域分割で補完の依存が減り、撃破上限が下がるか。

将来実装する際の、意味のある検査例は以下。

- 非空の真側または偽側を一つ消すと拒否する。
- 必要な比較そのものを飛ばすと、pcの不一致で拒否する。
- 根からFLEEやCRACKの証明を丸ごと消すと拒否する。
- HPの境界243を両方の子から落とすと拒否する。
- RNGの値または読み飛ばし個数を一つ変えると拒否する。
- カウンター後の敵HP式を元に戻すと拒否する。
- 消費済みMPを補完で払い戻すと、適用条件または更新式の不一致で拒否する。
- 出力の一部しか入らないセルを単一行き先と主張すると拒否する。

これはこれから行う検査の指定であり、本書作成時に通過した実測報告ではない。

時間は次の内訳で計上する。

```text
初回の局所記号実行
追加で再開した局所記号実行
ガードの制限と出力先への接続
証明ノードの独立検査
DPと価格選択
再実行による勝利確認
```

最終余裕だけでなく、証明ノード処理数、詳細葉数、補完項数、再開回数、
最大経路上の補完数、追加述語数、再利用した葉の割合を併記する。
DPの2,400万辺評価は、この中の一項の例でしかない。

命令列の準備と監査には開発上の労力が必要で、seedごとの局所証明の生成・検査には実行時間が必要である。
どちらも「既存Stepを呼ぶだけ」として見積もらない。
数百ms〜15秒に収まることは、これらの実測と、小さい分割で証明が締まるかの検証が済むまで未確定である。

**13. 現ブランチの調査結果と、PoCで固定する状態**

調査対象は現行の `BattleEmulator.cpp`、`Player.h`、`BattleInitialPlayers.h`、
`setting.h`、`lcg.cpp`、`camera.cpp` と、開始状態を渡す `main.cpp` の該当箇所である。
この節のyo2_be用の事実は、作業中の現行ファイルから確認した。
後に許可された指定ブランチ先端の調査は第17・18節に分ける。過去の研究レポートは根拠にしない。

PoCでは、固定能力値の味方一人と敵一体、本体仕様の8行動、現行の敵行動選択に限定する。
敵の選択肢は `VICTIMISER, HP_HOOVER, CRACK_ENEMY, ATTACK_ENEMY, MANAZASHI, PUFF_PUFF`。
内部のカウンター・回避・麻痺解除・行動不能も含めて、一ターンの規則を閉じる。
列挙値がヘッダにあるだけの別の技を、この集合に追加しない。

| 調査した処理 | 現ブランチで使える事実と、その限界 |
| --- | --- |
| `BasePlayers` と `setting` | 敵最大HP456、味方最大HP65、現在MP22。味方atk61・def66・speed40、敵atk56・def58・speed54。薬草の実初期値は7で、`setting::herbcount=8` を代入しない |
| `Main` と `callAttackFun` | 上の到達可能な呼出しに `sleeping=true` の更新がない。両者が眠っていない入力から、眠りは新たに発生しない |
| 能力値の更新 | `RecalculateBuff` は定義されているが、この一ターンの呼出し経路から呼ばれない。対象のatk・def・speedは固定。味方の防御倍率 `defence` は別で、毎ターン1.0へ戻りDEFENCEで0.5になる |
| HEAL・CRACK・薬草 | 実行時にそれぞれMP2、MP3、薬草1を消費する。対象行動にMP・薬草を増やす更新はない。入口の合法性を修正規則で保証する必要がある |
| `ProcessRage` | 敵HP228・114の通過条件は各HPと定数の比較になる。怒りの開始時に残り2〜4を代入する |
| `MANAZASHI` と味方行動開始 | 麻痺を付けると残り4。減算後0以下で解除表を読み、-3の判定は必ず解除になる |
| 必殺・アクロ | 必殺の生タイマーは毎ターン減算、アクロの生タイマーも指定の行動後に減算される。フラグが無効でも負へ減り続ける場合がある |
| `paralysisLevel` | 増加・解除時の0代入はあるが、値を読む `==3` の本体は空。現在の遷移や乱数消費を変えない。生の整数演算の安全性は別途確認する |
| `camera` | Playerを読まず、行動配列・カメラ値から乱数を消費する。先攻引数から挙動を推測せず、実際の行動配列と関数内の処理順を使う |
| 現行の格納形式 | ターン番号・カメラ等は `uint64_t NowState` 内のビットを使う。HP・MP・薬草・各フラグとタイマーは `Player` の個別フィールドにある。資源3byte＋制御2byteという全体形式のpack/unpackは現行ソースにない |

これらから、**証明用のターン境界の制御型**を次に固定する。

| 座標 | enumの全値 | 値数 |
| --- | --- | ---: |
| Charge | `OFF, ON(0), …, ON(6)` | 8 |
| Paralysis | `CLEAR, ON(-2), ON(-1), ON(0), …, ON(4)` | 8 |
| Acro | `OFF, ON(1), …, ON(6)` | 7 |
| Rage | `OFF, ON(1), …, ON(4)` | 5 |
| Inactive | `false, true` | 2 |
| Camera | `0, …, 5` | 6 |

`ON(0)` と `OFF` は別の値である。麻痺の `ON(-2)` も有効な状態異常である。
フラグをタイマーの正負から復元しない。この直積26,880通りを事前に列挙する必要はない。
一個のBoxなら、六つのmaskで全通りを表せる。

生の状態からこの型への写像を `alpha` とする。無効中のタイマー値は証明用modeから省く。
これは実状態の値を変更する操作ではない。再実行用の状態には生の値を保持する。
可逆な実状態の保存と、この情報を捨てる証明用写像を同一視しない。

縮約の根拠は次の通りで、RuleProgramとの対応表に登録する。

1. 必殺がOFFの間、減算後の `==-1` は既にfalseのフラグをfalseにするだけで、乱数を消費しない。
   再チャージはタイマー6を代入する。アクロ使用は必殺をOFF・生タイマー0にする。
2. アクロがOFFなら、減算後のタイマー値はアクロ失効の追加乱数を発生させない。
   再有効化ではタイマー6を上書きする。
3. 麻痺・怒りの無効中のタイマーは、その状態の処理に使われず、再有効化時に上書きされる。
4. 麻痺は `r→r-1` の後、非正なら表添字 `abs(r-1)` を使う。
   入力の有効範囲-2〜4から、この添字は0〜3。-3では `1.0 >= getPercent(...,100)*0.01` が常に真になり、CLEARになる。
5. 必殺は有効0から次のターン冒頭でOFFへ、アクロは指定の減算で1からOFFへ、怒りは敵行動後の減算で1からOFFへ移る。
   FLEE等でその減算箇所を通らなければ、対応する値を維持する。
6. カメラ0〜5からの各分岐は0〜5へ戻る。`5-counter` の引数も0〜5である。

各mode更新の表は、規則の定数・減算・比較からkernelが8入力値以下を再計算する。
生成器の自由な写像表を信用しない。OFF中の生タイマー等を省いてよいことのC++対応監査は、
第1節のRuleProgram作成者の責任である。その監査まで局所証明から自動的に得られるとは主張しない。
必要な対応は、合法性、資源と乱数位置、次のmode、終端判定について
`alpha(Step_raw(S,b)) = Step_mode(alpha(S),b)` となることである。

生の整数の未定義動作を縮約で隠さないため、入力ごとに残り予算Hを使って検査する。
必殺・アクロの生タイマーには、保守的な下界 `min(入力値,0)-H` がintの範囲内であることを要求する。
麻痺レベルには `max(入力値,0)+H` がintの範囲内であることを要求する。
これらは各ターン高々一回の減算・増加と、固定値へのリセットから得られる。
ターン番号・Geneの添字等も、使う再実行入口の範囲内であることを検査する。

途中局面を受け取る場合、HP・能力値・有効mode等を入力から確認する。
不正な値を近いenumへ丸めない。眠り中、有効麻痺タイマー-3、能力値不一致などは、
このプロファイルの対象外として `UNSUPPORTED_INPUT` を返す。
過去の戦闘開始からの到達可能性を逆に証明する必要はなく、指定された正確なS0からの命題を扱う。

FLEEの修正規則もPoC用に固定する。選択時は麻痺中なら不許可とする。
敵先攻後、実行時に麻痺またはinactiveなら通常の行動不能処理へ進み、解除判定・フラグ更新・必要な乱数を実行する。
行動可能なら味方の逃走選択部分を実行し、敵行動・ターン処理・カメラは維持する。
inactiveによる行動不能処理の適用も、このRuleProgramの明示的な修正点である。
無修正の `Main` を再実行して、この修正規則での勝利手順と呼ばない。

**14. Invariantを作る人と、保証する人を分ける**

v1は、不変条件の探索器を作らない。kernelは入力の最大HP・現在MP・現在薬草数と、
第13節の固定型から、次の一個のBoxを自分で作る。

```text
BaseBox(S0) = E:[1,456], A:[1,65], M:[0,M0], I:[0,I0]
              × 第13節の六つのmodeの全mask

0<=M0<=22。I0は受理したS0の実値で、PoCの標準初期状態では7。
本プロファイルでは0<=I0<=7を受理し、所持数を変更する問題は別の入力プロファイルにする。
```

M0・I0を上限にできる理由は、対象遷移に補充がないことである。
E・Aの下界1は継続状態だけに適用する。成功・失敗終端へは適用しない。
固定能力値・眠りなし・省略フィールドの条件は、mode化のプロファイル検査に含める。
`rage→E<228`、`paralysis→A<...` のような観測由来の相関を勝手に追加しない。

経過ターンtと乱数位置pは、次の有限な外包で管理する。

\[
\operatorname{Envelope}_t=
\{p_0\le p\le p_0+tR_{\max}\}\times\operatorname{BaseBox}(S_0),
\qquad 0\le t\le H.
\]

`Rmax` はダメージ上限ではなく、一ターンの乱数位置の増分上限である。
第3節で通常CFGとcall graphの有限・acyclicを検査した後、kernelがその後ろ向きDPで再計算する。
固定ループはunroll済み、カウンターは非再帰の固定CALLであり、nativeも登録済みの有限消費契約を使う。
登録済みの全entryのmaximum instruction steps・最大call depthも、この前提に基づく。
現コードでは、次の緩い積み上げで64を予算にできる。

```text
ターン冒頭                                     <= 3
味方行動前1 + callAttackFun 20 + 行動後2       <= 23
敵選択前後6 + callAttackFun 20 + 行動後1       <= 27
ターン末尾                                     <= 1
カメラの二枠                                   <= 6
合計                                           <= 60 <= 64
```

ここでcallAttackFunの20は、対象caseとその内部呼出しを含む保守上限である。
到達不能な見惚れ側も含めて選択部分を緩く数えている。
実装では表の64を無条件に信用せず、登録したRuleProgramについて消費上限を再計算する。
規則が変わって64を超えれば、対応するEnvelopeと配列容量を作り直す。
現LCGの配列は5000要素なので、PoCの簡単な入口条件は `p0>=1` かつ `p0+H*Rmax<=4999`。
読み飛ばしも含めて位置を確認する。範囲外の遷移を存在しない辺として捨てない。

**この外包を真だと仮定して根を削るだけでは不十分である。** 合格に必要な義務は次の三つ。

\[
\begin{aligned}
&\alpha(S_0)\in\operatorname{Envelope}_0,\\
&S\in\{p\}\times D_{t,p,\ell,b},\ \operatorname{Step}(S,b)=S',\ S'\text{が継続}
  \Longrightarrow S'\in\operatorname{Envelope}_{t+1},\\
&S'\text{が終端}\Longrightarrow E',A',M',I'\ge0
  \text{かつ、規則どおりの成功・失敗分類}.
\end{aligned}
\]

第一式は入力の値で直接検査する。第二・第三式は、第5節の全詳細葉と第15節の全補完に対して検査する。
詳細葉では四つの資源像の端点とmodeの像を検査するだけでよい。
MP・薬草が負になる葉を「Invariantの外なので無視」とすると、消費を無視した偽の証明になる。
成功葉についても非負を検査するのは、DPの証明が撃破時の `Phi(S')>=0` を使うためである。

必要な根は、次の前向き手順でkernelが決める。`ctx` は第16節の同一問題と不変な登録規則を保持する。
この手順を初回だけでなく、どれか一位置の `Partition[p]` をrefineした後にも呼ぶ。
入力partitionsは全位置の木とそのpartition_versionを固定したfamilyである。Kは今回の検査予算の上限とする。

```text
rebuild_support(ctx, H, partitions: PartitionFamily, envelope, proof_provider, checked_cache):
    S0 = ctx.Problem.S0; p0 = S0.p
    assert envelope == kernel_construct_envelopes(ctx, H)
    assert alpha(S0) in envelope[0]
    P_set = integer_range_inclusive(
        p0,
        p0 + H*ctx.bundle.bounds.Rmax)
    assert partitionsの位置キー集合 == P_set
    for p in P_set:
        validate_finite_partition_covers_BaseBox(partitions[p], max_leaves=K)
    Support[0..H] = empty_sets<CellKey>()  # 旧世代のbit列・local cell idをコピーしない。
    checked_coverage = empty_root_records(partitions.partition_version, この再構築用のcoverage_version)
    model = empty_layered_edges()
    Support[0] = { (p0, project(alpha(S0), partitions[p0])) }

    for t = 0 .. H-1:
        for (p,local_cell_id) in Support[t]:
            for b in ctx.bundle.profileの固定された8行動:
                D = envelope[t]のp断面 ∩ Cell(partitions[p],local_cell_id) ∩ Selectable(ctx,t,b)
                if D is empty: continue
                proof = proof_provider.required_root(ctx,H,partitions,t,p,local_cell_id,b,D,checked_cache)
                checked = verify_root(ctx,t,p,b,D,proof,partitions,envelope)  # 第5節。欠落は拒否。
                outputs = derive_all_checked_outputs(checked)
                for output in outputs:
                    if output is terminal:
                        資源の非負を検査して成功・失敗の終端へ分類
                        continue
                    assert output全体がenvelope[t+1]に入る  # 交差で削る前に検査
                    for p_out in outputが覆う全RNG位置:
                        assert p_out in partitions
                        if output is detailed:
                            destinations = classify_output(output.at(p_out), partitions[p_out])
                        else:
                            destinations = all_leaf_ids(partitions[p_out])  # 第15節の補完
                        全destinationsの(p_out,local_cell_id_out)をSupport[t+1]へ追加
                checked_coverageへこの根の検査記録を保存
                modelへ新partitionsで導いた全詳細辺・全補完を保存
    return (Support, checked_coverage, model)
```

詳細出力のatは具体的な一位置への参照で、classify_outputはその木の全非空逆像も返す。
補完はp区間を範囲として記録したままでよく、Supportには各pの全葉を加えても、DP辺を全組数へ展開する必要はない。
位置や葉の未定義はエラーであり、辺なし・空集合・負の無限大に置換してはならない。

生成時の `proof_provider` は、kernelから要求された根を、適合するキャッシュの領域制限、
不足部分の記号実行、または適用可能な補完で満たす。古いSupportにないことを欠落の理由にできない。
最終verifierでは同じ入口を提出証明書の読取専用providerに替え、必要な根がなければ拒否する。
いずれも次の根を決めるのはkernelである。生成器のSupportや「検査済み」印をそのまま受け取らない。

例えばPartition[100]の旧local cell 7の `A∈[1,65]` を、新7の `[1,32]` と新8の `[33,65]` に分けたとする。
検査済みの継続出力にp'=100で `A'=20` と `A'=60` があれば、次層の支持には `(100,7)` と `(100,8)` が必要である。
旧bit列の7だけを残すと8の必要根を落とす。別位置の `(101,7)` は独立した葉であり、この変更では分割しない。
local cell idを一つ改名することや、変更位置の旧Supportだけを置換することは、S0からの全層再構築にはならない。

Support・根一覧・セル接続の再利用キーは少なくとも
`(ctx.problem_key, H, partition_version, coverage_version)` とする。
partition_versionは**partition family全体**の実行内の世代番号で、一位置でも木を変更したら更新する。
変更のない木の不変データは共有できるが、旧世代のlocal cell id付き記録を現世代の根としてそのまま受理しない。
coverage_versionは検査対象の被覆・補完・行き先を変更するたびに更新する世代番号である。
生成時は再構築用の新しい世代を用意し、最終verifierは提出された世代を記録の照合にだけ使って内容を再検査する。
同じ番号の内容を変更しない。証明書は全partitionsを持ち、根記録はこのキーに `(t,p,local_cell_id,b)` を加えて識別する。
世代番号だけを信用するのではなく、最終verifierが木・根領域・全出力先を再計算する。
DPと区間最大表にはさらに現在の整数価格が対応する。規則のversionとは別の識別情報である。
構築途中のSupportやmodelはDPへ公開せず、全必要根を処理してから一組として差し替える。
再構築が時間・容量予算を超えたらUNKNOWNとし、古いSupportへ戻してFALSEを出さない。
同じ規則の詳細葉を部分領域へ制限して使える条件は第7節のままであり、戦闘実行をすべてやり直す指示ではない。

セル一個の支持は、その中の全状態が到達したという主張ではない。
全状態を根として多めに調べることは安全である。最初のセルでも同じ扱いにする。
支持を具体的な生存個体や代表HPから作ることはしない。
補完が広い行き先を要求した場合も、その支持を引き受ける。処理量を超えたら未判定にする。
H層目の継続状態は、その先の行動を検査する必要がない。

この手順についてtで帰納すれば、実際に到達する全継続状態がSupportとEnvelopeに入り、
そこから選べる全行動が覆われる。根・出力のいずれにも到達可能性を判定する未知の関数はない。
乱数位置の外包はtで変わるため、DPの層と対応させる。Hまたはpartitionsを変えた証明に古い支持集合をそのまま流用しない。
規則実行の葉そのものは第7節の条件で再利用できる。

| 担当 | 責任 |
| --- | --- |
| RuleProgram作成者・レビュー担当 | 現C++、明示した修正点、mode化、省略フィールド、native計算との対応を固定する |
| kernel | 登録時の有限実行性と静的上限の検査、入力検査、BaseBox・Envelope・合法領域の構築、全必要根の決定、保存性と終端資源の検査を行う |
| 証明生成器 | セルの切り方、展開箇所、価格、候補手順を提案する。Invariant・合法性・空判定は上書きできない |
| 呼出し側 | seed・正確なS0・残りターンの意味を固定し、同一問題の証明と手順だけを組にする |

さらに強い条件が必要なら、v1では許可された原子条件でセルを分割する。
新しい不変条件を足して根から状態を削るAPIは設けない。
したがって「良いInvariantを発見できれば動く」をPoCの前提にしない。

**15. 補完も、任意の上限関数にしない**

v1で `COMPLETE` を認めるpcは、次の四種類だけにする。
各pcは、規則に登録された行動後の処理まで終えている位置とする。
生成器が付ける `action_progress` の文字列だけで適用を許可しない。

| 中断箇所 | 残りの資源変化の上限 |
| --- | --- |
| ターン入口。コマンドは合法に選択済み | 第8節の行動別の表 |
| 味方行動とその行動後処理が終了し、敵行動が未実行 | 敵HP減少はカウンターによる最大64、味方HP増加なし、MP・薬草増分0。従って `64Q` |
| 敵行動とその行動後処理が終了し、味方行動が未実行 | 下の味方行動だけの表 |
| 両者の行動とその行動後処理が終了し、カメラ等だけが残る | 資源増分はすべて0 |

敵行動後、味方行動だけが残る場合の表は次の通り。

```text
ATTACK                 : 64Q
DRAGON                 : 34Q
DEFENCE / FLEE / ACRO   : 0
CRACK                  : max(0, 34Q - 3v)
HEAL                   : max(0, u*gH - 2v)
薬草                   : max(0, u*gI - w)
gH=65, gI=39 は第8節と同じ保守上限。
```

各 `max` は、未実行・実行の二つの資源収支ベクトルを明示的に持つという意味である。
補完書式を `cut_id, case_id, target_kind` とし、上限係数と出力外包はkernelが表から再構築する。
任意の式や任意の補題関数を証明書から読み込まない。
そのcutで必要な全case_idもkernelが列挙し、一つでも欠ければ拒否する。
caseを省略できるのは、kernelがその中断領域で不可能だと確認した場合だけである。
具体的なpcで既に生死が確定している場合は、まず終端として処理し、残りの攻撃機会を追加しない。

この表だけでは、出力がInvariant内にあることの根拠にならない。
補完には次の出力外包も必須であり、kernelが残りの処理の性質から作る。

1. 敵HPの下界は `max(0, 現在Eの下界 - 残り直接攻撃上限 - 残りカウンター上限)`。
   敵行動が残る場合の上界は456、味方行動だけなら現在Eの上界。
   HP_HOOVERによる回復を無視して上界を小さくしない。
2. 味方HPの下界は保守的に0。上界は回復実行の場合65、回復しない場合は現在Aの上界。
   残り資源変化が0の箇所では、E・Aとも現在の式をそのまま保つ。
3. MP・薬草は、対応する実行ケースでだけ現在の式からコストを引く。
   未実行ケースや、既に味方行動が済んだ箇所では現在の式を保つ。
   コストを引くケースでは、その中断領域全体で残量が足りることを検査する。
4. 制御値は第13節の各更新表の像を含める。粗い場合は全modeを使える。
   根拠は、HPのclampと非負ダメージ、各資源の書込箇所、有限modeの更新表であり、
   「出力もInvariant内のはず」という仮定ではない。
5. 乱数位置は `[pc時点のp, pc時点のp + Rremaining(pc,stack)]` で覆う。
   `Rremaining` は第3節で検査済みの有限CFG・非再帰のcall graphと、登録native契約に依存する。
   現在routineの残り上限と、kernelが構成したstackの復帰先の残り上限を足して求める。
   確定したprefixの消費と足して一ターンの上限に入ることも検査する。

Eの出力下界が正なら成功終端を行き先に加える必要はない。
下界が0なら、撃破不能と別途証明していない限り成功終端を含める。
生存する出力を分類するときだけE・Aの正の部分を取り、E=0の成功部分、E>0かつA=0の失敗部分も覆う。
資源が負になる部分を、この終端分類の名目で捨ててはならない。

補完の重みは第9節どおり、検査済みprefixの正確な収支上限と、上表の残り上限を足す。
従って、敵先攻のカウンターをもう一度与えたり、支払い済みのMPを戻したりしない。
HEALのMP消費直後等、表にない途中pcでの `COMPLETE` はv1では拒否する。
そのpcで予算に近づいたら、許可されたpcまでの処理を予約できる場合だけ進む。
予約できなければ未判定とする。任意pc用の補完エンジンをPoCの前提にしない。

行き先集合の書式も限定する。

```text
詳細葉       : Partition[p']の逆像で求めた単一の(p',local_cell_id')、または終端
補完         : union_{p' in 検査したp区間} {p'}×Leaves(Partition[p']) + 必要なら成功終端
最も粗い補完 : 次層の位置範囲について上のunion + 必要なら成功終端
```

失敗終端も被覆記録には残すが、成功へ至るDPの行き先にはしない。
補完のp区間は出力外包全体を含む必要がある。
その区間の外や、未登録のセルへ出る可能性を黙って削れない。
全セル最大は層ごとに一回求める。p区間最大を使うなら、まず各pについて
`max_{ell in Leaves(Partition[p])} B[h][(p,ell)]` を作り、
固定配列の区間最大表等で問い合わせる。その準備費・容量・問い合わせ数も計上する。
最大表を作る対象はその層の補完が要求する位置とし、そこでは全葉が対応するSupportとDPにあることを先に確認する。
不足するDP値を負の無限大で埋めて、補完の行き先を減らしてはならない。
例えばsparse tableなら層ごとに `O(P log P)` の準備と容量が必要であり、DPの辺評価数に隠さない。
複数の一次式・行き先集合の組は、実際の組数をCに数える。

PoCでのRuleProgram登録検査には、各caseの資源の書込先・消費回数・ダメージ上限、
mode表、pc別の行動済み情報と乱数消費上限を含める。
第13節の「生のタイマー値を省略してよい」という監査はこの登録の一部であり、
候補生成器の証明書から追加できる公理にはしない。
未登録の書込み・呼出し・caseが増えた場合は、この補完を再利用しない。
通常の詳細な記号実行へ進むか、そのプロファイルでは未対応にする。

**16. d(N−1)=偽 と d(N)=真 を組にして、時間を測る**

以後のNは、指定された正確なS0から最初の撃破までのターン数である。
観測済みのt0ターンを足して表示する場合は `t0+N` とし、固定された観測条件下での最短性と明記する。
この局面の結果を、別の開始局面や未固定の前半手順まで含む最短性へ一般化しない。

**rule_idは変更不可の登録versionとする。**
`rule_id = (registry_namespace, rule_version)` とし、一つの識別子は次の固定内容にだけ対応させる。
人間向けの表示名、ブランチ名、可変の「最新版」別名をrule_idの代用にしない。

| 固定する内容 | 登録に含めるもの |
| --- | --- |
| RuleProgram | 命令形式のversion、全routine・pc・分岐・定数・呼出し先、終了位置、明示的な修正規則。第3節の検査に用いる展開後の有限CFGとcall graph |
| 数値設定とnative対応 | 使用する乱数・ダメージ関数の固定実体、引数・副作用・有限終了・内部work・乱数消費の契約、整数幅・変換・丸め、浮動小数の演算順序、対象ビルドとコンパイル設定 |
| 対応profile | 資源軸・能力値・合法行動・入力制約、modeの値とbit対応、alphaと省略条件、外包の構築規則、補完の許可pcと登録表 |
| 正確な再実行との対応 | 使用する再実行入口と固定実体、ターン境界・開始ターン・行動指定の解釈、生状態とRuleProgramの対応表 |

これらを一つの `RuleBundle` とする。profileの名称だけ、関数名だけ、数値設定の自由な上書きは受け付けない。
例えばHPの比較を一箇所変更した場合も、乱数関数やmodeのbit対応だけを変更した場合も、新しいrule_versionを登録する。
現行の `Main` を指す入口と、FLEE修正規則を反映した入口は同じものとして登録しない。
数値設定は現在のC++の `double` という型名だけでは固定できず、実際に使う演算と関数群の対応確認が必要である。

```text
# 信用する規則の登録側の模擬コード。証明生成器からは呼べない。
register_rule(rule_id, reviewed_bundle):
    assert rule_id is not already registered
    assert reviewed_bundleのC++対応・修正点・native契約の監査が完了している
    candidate = deep_freeze(reviewed_bundle)           # 以降はこの同じ内容を検査する。
    check_instruction_types_and_resolved_targets(candidate.program)
    bounds = check_finite_execution(candidate.program, candidate.native_contracts)  # 第3節の全条件
    check_closed_program_and_profile(candidate, bounds)  # 全case、効果、mode、外包・補完表等
    registered = with_kernel_computed_bounds(candidate, bounds)
    registry.insert_new_only(rule_id, deep_freeze(registered))

# 判定要求の入口。TRUE再実行とFALSE検査へ、この同じctxを渡す。
bind_problem(Problem):
    bundle = registry.lookup_exact(Problem.rule_id)     # 未登録なら未対応
    assert bundleは第3節の登録検査を通過した不変な内容である
    assert 実際の実行入口・native関数・数値環境がbundleの固定内容と一致する
    validate_exact_input(Problem, bundle.profile)
    return frozen Context(Problem, bundle, problem_key=exact_problem_fields(Problem))
```

登録済みの内容を上書き・再割当てするAPIは設けない。別の登録表から同じrule_idを持ち込む場合も、
対応する構造化された登録内容を完全比較し、不一致なら設定エラーとして拒否する。
Rmax・最大call depth・各entryのmaximum instruction stepsはkernelが計算したboundsとして束縛する。
別実行からの登録データを読み込む場合にも第3節の検査と上限計算を行い、提出されたboundsを信用しない。
この照合は意味の同値性を自動証明するものではなく、監査済みの同じ内容を参照するための手続きである。
現状ではRuleProgram全体の実装・登録は未完了であり、本書の局所例に実際の登録済みversionを割り当てたとはしない。

**開始列Zからのroot作成と、候補を持たない探索の返却条件。**
本体仕様第13節の `extend_prefix(Problem_initial,Z,H_limit,limits,prove_minimal)` は、既知の勝利suffixを要求しない。
Problem_initialには固定seed、完全な生の開始状態、開始ターン、登録規則、観測・行動制約が必要である。
Zだけ、positionだけ、NowStateだけから他の値を推測してrootを作らない。

`exact_replay_prefix` の手順は次に固定する。

1. 不変なctx_initialの再実行入口を使い、Zの長さ、選択合法性、観測・行動制約、整数・RNGの入力範囲を検査する。
   prefix自体には残り長さ|Z|の安全性検査を行い、suffixは実際に得たS_ZについてH_limitの安全性を別に検査する。
2. Zの全コマンドを一ターンずつ正確に実行する。敵先攻による差替、カウンター、行動後処理を含め、
   継続する境界は規定の末尾・カメラ処理後とする。FLEE修正規則もsuffixと同じものを使う。
3. 観測の途中returnやMainのbool戻り値を、一ターン完了や勝敗の判定に代用しない。
   Zが途中の勝敗終端より後にもコマンドを要求する場合、勝手にZを短縮せずINVALID_PREFIXとする。
4. 完了した生状態S_Zのp、NowState、生のPlayer各値、ターン情報を保存する。
   `t_Z=t_initial+|Z|` と生のターン情報の対応は登録した再実行入口の定義に従って照合する。
   利用者が併せて渡した終端状態の主張があれば、これらの全フィールドと照合する。
5. `PrefixReceipt=(Problem_initial,Z,S_Z,t_Z,観測照合結果)` を、この実行内で検査した記録として作る。
   他実行から提出されたreceiptの「検査済み」表示は信用せず、必要なら同じ規則で再実行する。

空のZは恒等遷移である。Zの最終ターンで既に勝っていればsuffixは空列、最短suffix長は0である。
その時点で敵生存・味方死亡なら、suffixの不可能性は既存の自明な終端検査で扱う。
生存rootでH_limit=0なら同じく自明なFALSEであり、未実行の攻撃を足さない。
profile外の状態・未定義の効果・RNG範囲外はUNSUPPORTED_INPUT、prefixの不成立はINVALID_PREFIXとし、解なしの証明に置き換えない。

`bind_suffix_problem` は同じrule_id・seed・RuleBundleを維持して、S0をS_Z、開始ターンをt_Zにする。
将来制約の経過tは元の `t+|Z|` に対応させる。prefix中の観測はreceiptで確認し、残る制約を削除しない。
将来の制約評価にprefix情報が必要なら、登録済みの制約書式へその値も束縛する。表せない制約は未対応にする。
H_limitはsuffixだけの予算であり、|Z|を二重に引いたり、RNG配列に合わせて無断で縮めたりしない。

`attach_checked_prefix` は、receiptの終端S_Zとsuffix側Problemの生のS0、開始ターン、規則、seed、制約の接続を検査する。
一致した正確なprefix実行とsuffix実行を連結して `Z ++ W` とその根拠を返す。同一実行のこの接続検査で足り、
連結列全体の再実行をさらに行う場合は、その費用も同じ予算へ数える。
結果はZ固定・観測固定の問題についてのもので、Zを変更できる戦闘全体の最短性へ一般化しない。

| 正常な探索結果 | 受理に必要な根拠 |
| --- | --- |
| WIN(W) | Problem_Zの正確な再実行で、合法なWの最初の撃破長NがH_limit以下。prefixを返す入口では上記の接続検査も合格 |
| PROVED_FALSE(H_limit,C) | 同じProblem_Z・H_limitの既存の否定証明Cを独立検査。案内表や候補列挙の失敗だけでは受理しない |
| UNKNOWN(H_limit,report) | deadline・work・byte・候補上限、または進展できる候補と改善の枯渇により、上のいずれも確定しない |

入力・モデル・算術エラーはこの正常な三値と区別して返す。候補一列の違法性はその候補の失敗であり、入力Problem全体の不正とは限らない。
WINに同一問題のd(N-1)=偽が付いたものをOPTIMALと表示できる。既存の最短性の受理条件は次のままである。
最短性だけが予算切れになっても、既に正確に検査したWをUNKNOWNとして隠さず、WINと未確定の最短性を返す。

最短性の証明書は、次の二つと共通の入力識別情報を持つ。

```text
Problem = (rule_id, seed, 正確なS0, 開始ターンt0, 観測・行動制約)
TRUE    = Nターン以内のコマンド列と、同じ規則での再実行結果
FALSE   = H=N-1、全partitionsとpartition_version、被覆・補完とcoverage_version、整数価格、DP不等式
```

数値設定とprofileはrule_idの登録内容から得る。証明書に説明用の写しを持たせる場合も完全一致を要求する。
`exact_problem_fields` は固定書式で各フィールドを比較し、S0にはp・NowState・生のPlayer値を含める。
無効タイマーを省いた `alpha(S0)` で代用せず、構造体のpaddingやポインタのアドレスも比較対象にしない。
開始ターンはS0内のターン情報との一致も検査する。N・H・partitions・価格・探索予算は共通Problemには含めない。
refineはrule_idを変えず、第14節のpartition_versionと再構築したcoverage_versionを更新する。
補完の詳細化はcoverage_versionを更新する。価格だけの変更では両世代を維持し、価格に依存するDPを再計算する。

```text
ctx = bind_problem(requested_Problem)
if enemyHP(ctx.Problem.S0)==0: return OPTIMAL(0)
assert TRUE.Problemの全フィールド == ctx.problem_key
assert FALSE.Problemの全フィールド == ctx.problem_key
witness = exact_replay(ctx, TRUE.commands)             # 保存された勝利表示を信用しない。
if witness is not checked winning: reject
N = witnessの最初の撃破ターン
assert FALSE.H == N-1
checked_false = verify_false(ctx, FALSE)              # 提出partitions全体でSupportも再構築。
if checked_false accepted:
    return OPTIMAL(N)
```

別実行の「検査済み」フラグや表示名一致だけで組を受理しない。両側を同じ不変なbundleへ束縛して検査する。
表示名が同じでもrule_versionが違うTRUE/FALSEは組にせず、現在の規則で改めて再実行・再検査が必要である。

TRUE側は、各ターンの選択合法性、敵先攻後の実行時状態、乱数位置、最初の撃破を検査する。
近似したHPやmodeの代表値による再実行では合格にしない。
FALSE側は、第14節の被覆と保存性を検査した上で、

\[
\delta=QE_0-\bigl(B_{N-1}(q_0)+uA_0+vM_0+wI_0\bigr)\ge1
\]

または抽象成功経路が存在しないことを整数で確認する。
敵HPが0なら初めからN=0の自明な成功として扱い、`d(-1)` を問い合わせない。
味方死亡・H=0等の自明な偽も、同じ終端優先順位で検査する。
この二つがそろったときだけ、正確な問題について最短Nと返す。

近似と量子化の許可箇所を、次のように分ける。

| 箇所 | PoCでの扱い |
| --- | --- |
| 戦闘のHP・MP・乱数・ダメージ | 固定した規則の正確な値と演算を使う |
| HP帯やmodeのまとめ方 | 第2節のBoxによる外側近似。代表点へ丸めず、帯の全点の遷移を覆う |
| 証明価格 | 量子化してよい。初期設定は `Q=256`、`0<=u,v,w<=2^20` の整数 |
| 価格候補の探索 | 非信用部分なので浮動小数や小さいLPを使える。丸めた係数で必ず整数DPを再評価する |
| 最終判定 | 整数の厳密比較。浮動小数の微小な負の余裕、LPの終了表示、探索失敗を証明として受け取らない |

固定分母が粗くて余裕を失うなら別のQを候補にできるが、積・和を検査し、DPを再評価する。
価格候補の最適性を証明する必要はない。一組の係数で上の十分条件が成立すれば足りる。
浮動小数で求めただけのLP下界から、価格の族全体が不十分だと断定もしない。
本体仕様の厳密な有理数LPは拡張として残し、PoCの必須部分にはしない。

動作の順序は次に固定する。

```text
入力受理から共通のdeadlineを開始する
prefix Zがあれば正確に再実行し、同じ規則に束縛したProblem_Zを作る
既知の候補手順があれば、修正規則で再実行する
候補がない・候補不成立なら、本体仕様第13節のsolve_suffixを明示したH_limitで実行する
  WINなら最初の撃破長Nと勝利列Wを得る
  PROVED_FALSEならH_limit以内の不可能性を返す
  UNKNOWNならその探索範囲と打切り理由を返す

勝利手順が得られ、最短性も要求されたら H=N-1 の否定証明を作る
  整数の否定証明が検査に合格 → 手順と組にして OPTIMAL(N)
  より短い実手順が見つかる   → Nをその撃破長へ更新し、新しいH=N-1を調べる
  上限が締まらない           → 第11節の分割・再開・価格更新を予算内で続ける
  時間・容量不足             → Wと検査済みの上界・下界を保ち、最短性未確定のWINを返す
```

手順探索で見つからなかったことを偽の根拠にはしない。
既に `d(h)=偽` を証明し、長さNの勝利手順もあるがh=N-1まで届かなかった場合は、
`h+1<=最短ターン数<=N` を返せる。最短確定とは表示しない。
観測条件・seed・S0・RuleProgramが異なるTRUEとFALSEを組にしてはならない。

数百ms〜15秒を狙うPoCの開始設定を、次の累積予算とする。
これは実測値でも達成の保証でもなく、実装後に調整する初期の打切り設定である。
suffix探索では100msも測定点とし、100msのdeadlineを指定した実行も評価する。
既存の下表は保持し、100msの問い合わせには300ms行以下の容量・work上限と、指定した短いdeadlineを適用する。

| 入力からの累積時間 | K上限 | J上限 | C上限 | DP評価する価格候補の累積上限 | 総ノード・領域処理Vの累積上限 | 記録byte上限 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 300ms | 2 | 2 | 2 | 2 | 50万 | 16MiB |
| 2秒 | 4 | 4 | 3 | 4 | 200万 | 64MiB |
| 15秒 | 8 | 8 | 3 | 8 | 800万 | 128MiB |

実際の詳細・補完項数にも、順に6.5万・15万・25万の総上限を置く。
Kは各 `Partition[p]` のleaf数の上限であり、J・Cは各CellKey・行動に対する上限である。
いずれも全乱数位置で使い切る指示ではない。
RNG位置数Pは `H*Rmax+1` 以下の外包から支持を絞り、Hを20に固定しない。
価格候補数は、グラフを変更しても累積予算をリセットしない。
本体仕様第13節の候補2048列・選択肢走査200万回・改善16回は、これらに追加する独立の累積上限である。
候補の再実行、L・行き先索引、重複比較、改善後の全層再構築もV・byte・時間の内数とし、別枠の15秒は設けない。
任意の新規インスタンスを内部で作って累積カウンタを初期化せず、一つのBudgetをprefixから最短化まで渡す。
節目で証明できたら直ちに返し、最終段階まで必ず反復する必要はない。

各処理は小さい単位で予算を確認し、最終検査と手順再実行に必要な時間を残す。
最終検査を打ち切った場合、その候補はFALSEとして受理しない。
同じkernel実行内で独立検査済みの葉は再利用できるが、生成器や以前の別実行の「検査済み」印は信用しない。
決定的な連続命令はまとめて走査してもよい。記録の圧縮で各命令の処理数Vまで減ったことにはしない。

時間報告には、入力・乱数テープの準備、手順取得、局所証明の生成、kernel検査、
領域接続・支持集合・区間最大表、全価格のDP、最終再実行を含める。
探索ではprefix再実行時間、最初のWINまでの時間、候補数・重複skip数・仮想行き先走査数・改善回数も分けて報告する。
候補なしでのWIN率、PROVED_FALSE率、UNKNOWN率を100ms・300ms・2秒・15秒で測り、既知witnessをヒントに渡した測定と分ける。
キャッシュなしの時間と再利用時の時間を分け、H・P・実セル数・実項数・V・総byte数を併記する。
コード中の古い手順コメントや、入力・seedが未確定の観測例を、検査済みの最短性データとしない。

実装時の追加の拒否検査は、次に絞る。

- BaseBoxから合法なMP値やmodeを削ろうとする証明書を受理しない。
- 生の無効タイマーが異なる二状態は、定義域内なら同じmode遷移になる。麻痺の負の有効タイマーはCLEARと区別する。
- 出力をEnvelopeで切り落とす証明、成功時にMPが負の証明を拒否する。
- 補完の行き先に含まれるセルをSupportから一個消すと、必要な根の欠落として拒否する。
- セル分割で葉番号が再使用されても、新しい両出力先からSupportを再構築し、片方の必要根を欠く証明は拒否する。
- Partition[100]だけをrefineした際に他位置の木が変わらず、p'への出力がPartition[p']で分類されることを確認する。位置違い・旧partition_versionのlocal cell idによる根や接続の流用は拒否する。
- ある位置のleaf数がKを超える木、未定義の出力位置、全層再構築で必要になる根の欠落を拒否する。
- CFGのcycle、未解決jump、未登録CALL先、自己・相互再帰、未処理の末端、誤ったRETURNを登録時に拒否する。無消費のcycleも拒否対象である。
- nativeの有限終了・work・RNG上限が欠ける登録を拒否し、手書きのRmaxを小さくしてもkernelの再計算をすり抜けないことを確認する。
- unrollした二枠・9比較・非再帰COUNTERの順序と、最大命令数・call depth・Rremainingの算出を確認する。COMPLETEを加えて不正なProgramを登録可能にはしない。
- 表示名が同じでRuleProgram・数値設定・profileのいずれかが異なる登録versionの組を拒否する。同じversionへの上書きも拒否する。
- 許可されていない途中pcの補完、済んだカウンターの追加、消費済みMPの払い戻しを拒否する。
- 量子化前だけ不等式が成立する候補や、別の入力・規則の勝利手順との組を拒否する。

探索PoCでは、正確な初期問題とZ・H_limitだけを渡し、外部の勝利候補なしでWを生成・再実行して返せるかを評価する。
先頭候補が架空でも別候補へ進むこと、改善不能・予算切れをUNKNOWNにすることも評価条件に含める。
最短性PoCの合格条件は、正確な入力ごとの `d(N-1)=偽` と `d(N)=真` の組を、
予算内で独立検査できることである。判定できた問題の割合と、未判定の理由も測る。
今ある資料だけでは、この組や300ms〜15秒の達成を報告する段階にはない。
必要なのは、以上の固定形式で小さい証明が実際に作れるかという実測である。

**17. 指定された前半系2例で、効果の型と減算位置を確認する**

前回の追加調査で読んだソースは、次の4ファイルだけである。ブランチの先端を読み、過去の版へは遡っていない。
今回のSupport・rule_idの補足では、この二例の再取得は行っていない。
今回の位置別分割・有限実行性・責務分割の補足でも、以下の横展開用の前提設計は維持する。
約17ブランチへの展開を見据えた条件であり、yo2_beの固定値へ統一したり、全ブランチを対応済みとしたりしない。

| 参照した先端 | 指定ファイル |
| --- | --- |
| 前回調査時のローカル `bilyouma_new_arugo` の先端 | `BattleEmulator.cpp`, `Player.h` |
| 前回調査時の `origin/zilyadama_new_arugo` の先端 | `BattleEmulator.cpp`, `Player.h` |

探索アルゴリズム、指定外のヘッダ・初期値・乱数・カメラ実装、`deep*.md` は追加調査していない。
以下は指定4ファイルから分かる拡張条件であり、各ブランチ全体の登録検査が済んだという意味ではない。
特にbilyoumaの `EnemyActionCandidates` の定義は指定外ヘッダにあり、今回の読取範囲にはない。
合法行動集合や初期能力値を、caseの存在やコメントだけから確定させない。

`--------start_FUN_021594bc-------` と `--------end_FUN_021594bc-------` の間を調べると、
同じマーカー名でも、敵行動後と味方行動後で実行内容が異なる。

| 効果 | 付与・更新 | 減算・解除の位置 |
| --- | --- | --- |
| bilyoumaの守備力低下 | KASAP成功時に味方 `BuffLevel--`, `BuffTurns=7`、その後 `RecalculateBuff` | 両者生存かつ味方の非skip行動後。守備力→素早さ→アクロの順 |
| bilyoumaの素早さ低下 | DECELERATLE成功時に味方 `speedLevel--`, `speedTurn=7`、同じ再計算 | 上と同じ味方行動後。先攻判定は、そのターン冒頭に済んでいる |
| bilyoumaの眠り | SWEET_BREATHの成功で `sleeping=true, sleepingTurn=2` | 味方行動開始時に減算・自然解除。敵の物理攻撃が非零ダメージなら途中でも解除 |
| zilyadamaの敵スカラ | BUFF_ENEMYで敵 `BuffLevel!=2` なら段階を増やし、`BuffTurns=7`、敵の守備力を再計算 | 両者生存なら**敵の各行動後**。敵が2回動くターンは2回減算しうる |

bilyoumaの敵行動後のマーカー部分には、上の味方バフの減算はない。
敵の二つの行動は先に選ばれ、その後まとまって実行される。味方行動の間に一つずつ置き直さない。
FLEEや、眠りから敵先攻中に起こされて味方の行動枠がskipになる経路は、減算箇所を通らない場合がある。
従って、状態の種類だけで「ターン末尾に一回減らす」と決めてはならない。
RuleProgramには効果の所有者、減算pc、その時点の生存条件、残り行動枠を含める。

指定先端のバフ自然解除表は `{62,75,87,100}` で、減算後 `r<=0` のとき

```text
v = table[abs(r)]
解除条件 = v >= randomPercent + (v==75 ? 1 : 0)
```

となる。眠りは `{37,62,87,100}` と `v>=randomPercent` を使う。
`randomPercent ∈ {0,...,99}` を登録条件とした局所的な全値確認では、
バフの各行で条件を満たす値は63・75・88・100個、眠りでは38・63・88・100個である。
表の整数を、そのまま一般的な百分率比較へ書き換えない。
減算後-3では必ず解除されるので、この条件下で継続する有効状態の下限は-2になる。
将来の `T,...,0,-1,...,-4` への対応は残すが、指定先端の有効値域を根拠なく-4まで広げない。
無効化後の生タイマーが-4以下になることとは区別する。

拡張するmodeは、フラグ・段階・タイマーを別々の独立変数にせず、次の有限型とする。

```text
EffectMode = OFF | ON(level, remaining)
SleepMode  = CLEAR | ON(remaining)

候補の型：
  味方DefDown / SpeedDown : OFF または level∈{-2,-1}, remaining∈[-2,7]  → 各21値
  敵DefUp                : OFF または level∈{1,2},   remaining∈[-2,7]  → 21値
  bilyoumaのSleep         : CLEAR または remaining∈[-2,2]              → 6値
```

これらは初期条件と保存性を検査するための候補型である。
bilyoumaの低下caseには最小段階へclampする代入がなく、敵行動選択の置換が下限を支えている。
その置換と二枠の重複処理まで含めて保存性を検査して初めて、level>=-2を根の制限に使える。
zilyadamaは0・1からの増加と2で増やさないcaseを検査する。
不一致なら入力や出力を型へ丸めず、プロファイルを修正するか未対応にする。

PoCの最大8値のmask・写像表を、これらの効果では32bit mask・最大21入力の表等へ広げればよい。
両符号の段階や-4を含めて値数が増えた場合は、宣言した有限サイズに合わせる。
集合の操作は引き続きAND・包含・有限表の像と逆像であり、任意述語の関数は追加しない。
無効タイマーを省く際の生整数の検査は、所有者の最大更新回数を使う。
例えば敵スカラの保守的な減算回数はHではなく2Hになる。

実効守備力・素早さは、段階と固定の基準能力値から得る**派生値**として扱える。
入力時の一致と、全書込みで再計算されることを対応監査する。
bilyoumaの `RecalculateBuff` は味方の守備力と素早さを両方更新する。
zilyadamaの同関数は指定した側の守備力を更新し、素早さ更新はコメントアウトされている。
名前が同じという理由で同じnative命令として流用しない。

先攻判定やダメージ計算では、必要な実効値が一致するmodeを一群にして分割する。
例えばDefDownの21値は、守備力を読む箇所なら通常・一段低下・二段低下の最大3群で足りる。
同じ段階の全タイマーを一つずつ具体化する必要はない。
減算や解除では、そのpcの比較結果が変わる群を分け、全非空群を検査する。
この扱いなら、資源の更新形式を変えずに状態異常を追加できる。

**18. 二例から分かった、効果の追加だけでは済まない点**

**行動順序を、ターン境界の状態と局所Frameで分担する。**
zilyadamaの敵行動回数は `((counterJ & 1)+1)` で、戦闘の奇数ターンに2回、偶数ターンに1回である。
開始ターンt0と経過tから実行ターン `t0+t+1` を復元し、残り予算Hの奇偶で代用しない。
同じ乱数位置・同じ効果modeでも、行動回数が違えば局所証明は共用できない。
DPはその層の実行条件に対応する辺を使い、キャッシュにもこの条件を含める。

同ブランチの `FUN_0208aecc` は `NowState` の4〜7bitを読み、3なら0として、
乱数1bitと組み合わせて行動表の0/1、2/3、4/5の組を順に選ぶ。
このローテーション値はターンをまたぐため、Cameraとは別の制御座標が必要である。
スカラ上限かつMP0で先頭の組を実行できない場合、選択ループは行動数を増やさず再選択する。
既にローテーションが次の組へ進むので、有効な0〜3の値と指定の行動表では再試行は一回以下と確認できる。
`counterstore` による見惚れ判定の省略と、再選択の乱数も規則に残す。
一般的な無限ループとして丸投げすることも、一回の失敗選択を一ターンとして数えることもしない。

bilyoumaの行動置換ループは、眠り→守備力低下→素早さ低下→通常攻撃という一方向の置換と、
二枠の重複回避を含む。この局所ループの停止条件も、実際の候補表を登録した上で確認する。
これらの横展開でも、第3節の登録条件を満たすよう、確認した再選択・置換回数でunrollする。
展開後に再試行が残りうる場合は登録拒否とし、その経路を削って有限に見せない。
局所Frameには、選択コマンド、予約済み行動、現在の行動枠、残り敵行動、必要なコンボ情報を残す。
bilyoumaでは敵先攻中に眠りから起こされても、予約されたSLEEPINGの枠をskipする処理がある。
「現在起きている」だけを見て、最初に選んだ攻撃へ戻してはならない。
コンボは同ブランチで実際にダメージを変えるが、zilyadamaの `processCombo` はそのまま値を返す。

**MP吸収とMP回復を、従来の資源不変条件へ押し込まない。**
zilyadamaのDRAIN_MAGICは `M'=max(0,M-6)`。閾値で分割すれば自座標の平行移動か定数になり、Boxで扱える。
ただし、敵先攻でMPを失うことは入口の合法性だけでは処理できない。
例えばMP2でHEALを選び、先に吸収されて0になった後、読んだHEALのcaseは無条件にMP2を引く。
これをそのまま実行するとMPが負になり、第14節の非負条件を満たさない。
横展開のRuleProgramでは実行時の不足を再判定し、不足時の行動と乱数消費を明示した修正規則へ進める必要がある。
不足領域の枝を削ることは禁止する。不足時の正式な規則が未固定なら、その拡張問題の判定は未対応とする。
今回、推測した不足時の処理を実装したり正本にしたりはしない。

両ファイルにはMAGIC_WATERのcaseがあり、所持数を減らして `M'=min(maxMp,M+g)` とする。
これを合法行動集合に入れるなら、`M<=M0` と「MPは増えない」という補完の前提を変更する。
Mの外包は `0<=M<=maxMp` にし、特薬草・特どくけしそう・まほうのせいすい等の
採用した消耗品の数を、それぞれ独立した非負の資源座標 `I_j` として持つ。
MP回復とアイテム消費を一つのケースとして上限化する。
caseがあるだけでは合法行動に追加せず、対象の行動集合を問題の入力プロファイルで固定する。

MIRACLE_SLASHの回復は、この二例では確定した基礎ダメージから計算され、MP4の消費とHPのclampを伴う。
現在敵HPそのものから回復量を求めているわけではないので、必要な能力値・乱数を具体化した葉では自座標形式に収まる。
一般に、現在HP比例のダメージや資源間の転送が増えれば、同じ形式に閉じるかを別途検査する。

**毒・複数の敵行動を含めて、補完の適用箇所を作り直す。**
bilyoumaには毒付与と、ターン末尾の `maxHp>>4` の味方HP減少がある。
maxHpが固定なら定数減算とclampで扱えるが、両者の行動終了時点ではまだ資源更新が残る。
第15節の「カメラだけが残り資源増分0」という補完を、毒処理の前へ移してはならない。
`PoisonTurn` はこのコードでは増加する経過カウンタで、負へ減る解除タイマーとは別の型である。
省略するなら読み書きの用途と有限予算内の整数安全性を確認し、毒の有効フラグと資源更新は残す。

敵が二回攻撃できる場合、カウンターの機会も二回ありうる。
行動別の上限は `残り直接攻撃上限 + 残り敵行動数×カウンター上限` から再検証する。
yo2_beの64・128、味方最大HP65、乱数上限64を他ブランチの定数にしない。
完了した敵一行動と、敵の全行動が完了した状態を区別し、各行動後のバフ解除を含めてpcを登録する。
指定先端だけでは未確認のカメラ・乱数関数・初期値を含む全体の消費上限を、今回確定したとはしない。

数値計算の設定もRuleProgramの識別に含める。bilyoumaのダメージ関数には通常ビルドの固定小数点版と
Emscriptenのdouble版があり、コンパイル定義で必殺率・基準HPも変わる。
両版が同じだという証明なしに混用しない。これは証明価格を量子化してよいこととは別の条件である。

共通で維持するものは、有限の宣言から作る外包、全必要根の被覆、像の包含、保存則の不等式、
同じ規則の再実行と組にした `d(N-1)=偽, d(N)=true` の受理手順である。
資源を増やしたときの保存則と計算量は、本体仕様の第18節に定める。
先にyo2_beの六つのmodeと四資源でこの組と実用時間を確認し、その後に前半系のプロファイルを追加する。
後半系の探索方式をこの方式へ変更することは、本仕様の作業範囲に含めない。
