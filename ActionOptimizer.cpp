...
constexpr ActionEntry ACTION_TABLE[] = {
	...
	},
	{
		...
		[](const Genome& g){
			return g.AllyPlayer.InsulateLevel <= 1 && g.AllyPlayer.InsulateTurns <= 2 && g.AllyPlayer.mp >= 10;
		},
		[](const Genome& b, const Genome& a){ return true; }
	},
};
		...
		while(!openSet.empty() && (maxGenerations == -1 || counter < maxGenerations)){
			...
			for(const auto& entry : ACTION_TABLE){
				...
				const bool currentlyBareHands =
					currentGenome.AllyPlayer.defaultATK == BattleEmulator::GANASADAI2_BARE_HANDS_ATK;
