...
{ BattleEmulator::DOUBLE_UP,
    ...
    { BattleEmulator::PSYCHE_UP_ALLY,
        ...
        [](const Genome& before, const Genome& after) { return after.AllyPlayer.TensionLevel > before.AllyPlayer.TensionLevel;}
        // 事後: テンションが上がったか
        ...
        while (!openSet.empty() && (maxGenerations == -1 || counter < maxGenerations)) {
            ...
            for (const auto& entry : ACTION_TABLE) {
                ...
                const bool currentlyBareHands =
                        currentGenome.AllyPlayer.defaultATK == BattleEmulator::ZUO_BARE_HANDS_ATK;
