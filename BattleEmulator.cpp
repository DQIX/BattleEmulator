...
constexpr int kaisinnP = 500;
constexpr int baseHP = 135;
constexpr double ShieldGuardP = 4.5;
constexpr double mitoreP = 0.1110;
constexpr int Ally_Level = 22;
#endif
...
bool BattleEmulator::Main(int* position, int RunCount, const int32_t Gene[350], Player* players,
    ...
    for(int counterJ = startPos; counterJ < RunCount; ++counterJ){
        ...
        const bool stunCarriedIntoTurn = players[0].isStunned;
        bool equipmentChangedThisTurn = false;
        bool bareHandsThisTurn = players[0].defaultATK == SILYARUMANA_BARE_HANDS_ATK;
        if(genePosition != -1 && Gene[genePosition] != 0 && Gene[genePosition] != -1){
            ...
            const int requestedDefaultATK =
                requestedBareHands ? SILYARUMANA_BARE_HANDS_ATK : SILYARUMANA_EQUIPPED_ATK;
            if(!players[0].paralysis && !players[0].sleeping && !stunCarriedIntoTurn &&
        ...
        while(counter != 2){
            ...
            if(enemyAction[counter] == DAZZLE && players[0].DazzleLevel >= 1){
                enemyAction[counter] = WHIRLWIND;
            }

            if(preAction != 0 && counter == 1 && enemyAction[0] == enemyAction[1]){
                if(enemyAction[1] == MAGIC_BARRIER){
                    enemyAction[1] = ATTACK_ENEMY;
                }
                else if(enemyAction[1] == BOOM){
                    enemyAction[1] = ATTACK_ENEMY;
                }
                else if(enemyAction[1] == WHIRLWIND){
                    enemyAction[1] = BOOM;
                }
                else if(enemyAction[1] == DAZZLE){
                    enemyAction[1] = WHIRLWIND;
                }
            }
            ...
            if((t == 0 && !player0_has_initiative) || (t == 1 && player0_has_initiative)){
                ...
                if(!skipTurn){
                        ...
                        if(action == SPECIAL_MEDICINE || action == SPECIAL_ANTIDOTE){
                            if(mode != -1 && mode != -2){
                                ...
                                if(currentDamage != InputBuilder::TYPE_SPECIAL_MEDICINE &&
                                    ...
                                    currentDamage != (players[0].maxHp - players[0].hp)){
                                    return false;
                                }
                                if(damages[exCounter] == -1){
                                    startTurn = counterJ;
                    ...
                    if(Player::isPlayerAlive(players[0]) && Player::isPlayerAlive(players[1])){
                        ...
                        players[0].DazzleTurns--;
                        if(players[0].DazzleLevel != 0 && players[0].DazzleTurns <= 0){
                            //0x02159f20 Dazzle
                            const int probability[4] = {37, 63, 87, 100}; //100以外全部.5
                            auto probability1 = probability[std::abs(players[0].DazzleTurns)];
                            auto probability2 = lcg::getPercent(position, 100);
                            if(probability1 >= (probability2)){
...
double BattleEmulator::FUN_021dbc04(int baseHp, int maxHp){
    auto hp = static_cast<double>(baseHp);
    if(hp == 0){
        return 0;
    }
    return hp / static_cast<double>(maxHp);
}
    ...
    switch(Id & 0xffff){
        ...
        case DAZZLE: {
            ...
            if(lcg::getPercent(position, 100) < 62){
                ...
                players[defender].DazzleTurns = 5;
                players[0].DazzleLevel++;
            }
        ...
        case MULTITHRUST: {
            ...
            for(int i = 0; i < attackCount; ++i){
                ...
                // if(lcg::getPercent(position, 100) < 0){
                //     kaihi = true;
                // }
                // else{
                //     (*position)++; //盾ガード 0x021586fc 0%
                // }
                (*position)++; //みかわし 0x021587b0 0%
                (*position)++; //盾ガード 0x021586fc 0%
                (*position)++; //ニセ回避 0x02157f58 100%
                if(players[attacker].DazzleLevel != 0){
                    if(lcg::getPercent(position, 8) < 5){
                        dazzle = true;
                    }
                }
                if(!dazzle){
        ...
        case PSYCHE_UP_ALLY:
            ...
            if(players[attacker].TensionLevel < 3 || (players[attacker].TensionLevel == 3 && lcg::getPercent(
                    position, 2) ==
                0)){
                //0x02087fb4 テンション
                players[attacker].TensionLevel++;
            }
            if(!players[0].specialCharge && !players[0].sleeping && !players[0].paralysis && !players[0].isStunned){
        ...
        case BattleEmulator::DOUBLE_TROUBLE:
            ...
            for(int i = 0; i < 2; ++i){
                if(preHP[defender] > totalDamage){
                    ...
                    if(kaihi || tate){
                        if(!players[0].paralysis && !players[0].sleeping && !players[0].specialCharge && !players[0].isStunned){
                            (*position)++; //0x021ed7a8
                        }
                        baseDamage = 0;
                    }
        ...
        case BOOM:
            ...
            baseDamage = FUN_021e8458_typeD(position, 8, 43);
        ...
        case WHIRLWIND:
            ...
            if(lcg::getPercent(position, 100) < 38){
                ...
                baseDamage = FUN_0207564c(position, players[attacker].atk, players[defender].def);
                if(baseDamage == 0){
                    baseDamage = lcg::getPercent(position, 2); //1a8
                }
                if(baseDamage != 0){
                    (*position)++; //0x021e54fc
                }
        ...
        case BattleEmulator::POISON_ATTACK: {
            ...
            if(tate || kaihi){
                ...
                if(baseDamage != 0){
                    int damage = static_cast<int>(
                        lcg::getPercent(position, 2) + 1.0 + baseDamage * 0.1
                    );

                    players[defender].mp = std::max(0, players[defender].mp - damage);
                    (*position)++; //不明
                }
            ...
            if(baseDamage != 0){
                (*position)++; //0x021e54fc おそらくシャルマナ限定
            }
            process7A8(position, baseDamage, players, defender);
            break;
        }
...
constexpr std::array<int, 6> ids = {
    ...
    BattleEmulator::ATTACK_ENEMY,
    BattleEmulator::BOOM,
    BattleEmulator::WHIRLWIND,
    BattleEmulator::DAZZLE
};
...
void BattleEmulator::RecalculateBuff(Player* players){
    ...
    if(index >= 0 && index < 5){
        players[0].atk = static_cast<int>((players[0].defaultATK * ATKMultipliers[index1]));
    }
}
