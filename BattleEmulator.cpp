...
constexpr double mitoreP = 0.2040;
...
bool BattleEmulator::Main(int *position, int RunCount, const int32_t Gene[350], Player *players,
    ...
    for (int counterJ = startPos; counterJ < RunCount; ++counterJ) {
        ...
        bool bareHandsThisTurn = players[0].defaultATK == GANASADAI2_BARE_HANDS_ATK;
        if (genePosition != -1 && Gene[genePosition] != 0 && Gene[genePosition] != -1) {
            ...
            const int requestedDefaultATK =
                requestedBareHands ? GANASADAI2_BARE_HANDS_ATK : GANASADAI2_EQUIPPED_ATK;
            if (!players[0].paralysis && !players[0].sleeping && !inactiveCarriedIntoTurn &&
        ...
        while (counter != 2) {
            ...
            if (state == TYPE_2A) {
                ...
                }else if (c == CRITICAL_ATTACK  || c == MERA_ZOMA) {
                    if (!players[1].rage) {
                        (*position)++;
                    }
                }else if (c == DISRUPTIVE_WAVE) {
                ...
                if (c == MAGIC_BARRIER && players[1].BarrierLevel == 2) {
                    c = ATTACK_ENEMY;
                }

                if (c == MAGIC_BARRIER && players[0].hasMagicMirror) {
                    c = ATTACK_ENEMY;
                }
                ...
                constexpr int table[6] = {
                    ATTACK_ENEMY, KABUFF,
                    INFERNO, ATTACK_ENEMY,
                    SWITCH_2A, SWITCH_2A,
                };

                c = table[rand];

                if (c == KABUFF && players[1].BuffLevel == 2) {
                    c = ATTACK_ENEMY;
                }
        ...
        for (int t = 0; t < 2; ++t) {
            ...
            if ((t == 0 && !player0_has_initiative) || (t == 1 && player0_has_initiative)) {
                for (int c: enemyAction) {
                    ...
                    if (c == MERA_ZOMA && players[0].hasMagicMirror) {
                        Player::reduceHp(players[1], basedamage);
                    }else if ((c == KAZAM) && players[0].hasMagicMirror) {
                        Player::reduceHp(players[1], basedamage);
                    } else {
    ...
    switch (Id & 0xffff) {
        case MAGIC_BARRIER:
            {
                ...
                tmp = baseDamage * (1.0 - (players[1].BarrierLevel * 0.25));//マジックバリア なぜかこの攻撃はマジックバリアの対象らしい。は？？？
        ...
        case KAZAM:
            ...
            if (players[defender].hasMagicMirror) {
                ...
                baseDamage = FUN_021e8458_typeD(position, 30.0, 112.0);
        ...
        case BattleEmulator::INSULATE:
            ...
            if (!players[0].specialCharge && !players[0].sleeping && !players[0].paralysis && !players[0].inactive) {
        ...
        case MERA_ZOMA:
            ...
            if (players[0].hasMagicMirror) {
                ...
                tmp *= 1.25;//火属性1/25倍
                tmp *= 1.0 - (players[1].BarrierLevel * 0.25);//マジックバリア
                tmp = processCombo(Id & 0xffff, tmp, NowState);
                baseDamage = static_cast<int>(floor(tmp));
                if (!tate) {
                    ProcessRage(position, baseDamage, players, preEnemyHp);
        ...
        case CLAW_SLASH_A2:
            ...
            if (kaihi || tate) {
                ...
                tmp = static_cast<double>(baseDamage);
                if ((Id & 0xffff) == CLAW_SLASH) {
                    tmp = floor(tmp * 1.25);
                }else if ((Id & 0xffff) == CRITICAL_ATTACK) {
                    tmp = floor(tmp * 2.5);
                }
...
void BattleEmulator::RecalculateBuff(Player *players) {
    ...
    if (index >= 0 && index < 5) {
        constexpr double DEFMultipliers[] = {0.25, 0.5, 1.0, 1.5, 2.0};
    ...
    if (index1 >= 0 && index1 < 5) {
        constexpr  double ATKMultipliers[] = {0.5, 0.75, 1.0, 1.25, 1.5};
        players[0].atk = static_cast<int>(floor(players[0].defaultATK * ATKMultipliers[index1]));
    }
    int index2 = players[0].SpeedLevel + 2;
    if (index >= 0 && index < 5) {
        constexpr double SPEMultipliers[] = {0.25, 0.5, 1.0, 1.5, 2.0};
        players[0].speed = static_cast<int>(floor(players[0].defaultSpeed * SPEMultipliers[index2]));
    }

    // 定数の倍率を格納した配列
    // BuffLevel に +2 して配列インデックスに変換
    int index4 = players[1].BuffLevel + 2;

    // インデックスが範囲外でないかチェック（-2 <= BuffLevel <= 2 の範囲であることを確認）
    if (index4 >= 0 && index4 < 5) {
        constexpr double DEFMultipliers[] = {0.25, 0.5, 1.0, 1.5, 2.0};
        players[1].def = static_cast<int>(floor(players[1].defaultDEF * DEFMultipliers[index4]));
    }
}
