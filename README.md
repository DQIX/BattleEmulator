...
> 配布先が変更(Dropbox)され、githubリリースはでの配布はすでに放棄されています。  
...
### Q: virtual studioでバトルエミュレーターをデバッグすると、耐えれないぐらい遅いんだけど

virtual studioを最新版に更新したうえで、試験的な`/dynamicdeopt`スイッチを使ってください。    
有効にするには、CMakeLists.txtに次のオプションを追加して、virtual studioのui上で**リリースモード**にしてデバッグを開始します。

```cmake
add_compile_options(/dynamicdeopt)
```

`/dynamicdeopt`スイッチにより、速度がほぼリリースバイナリと同じ速度でデバッグできるようになります。
...
### Q: msbuild版ビルドに付属してるpdbファイルってなに？
...
### Q: ソースコードイシュダルのものしか見れないんだけど
dq9BattleEmulatorCollection.cpp では、gitの機能である「ブランチ」分けを使用して、複数のコードを同時に運用しています。  
切り替えるには、パソコンでは、ページの一番上、左側にある「yo2」と書かれたドロップダウンで表示されるブランチをクリックすると、そのバトルエミュのソースコードを見ることができます。
